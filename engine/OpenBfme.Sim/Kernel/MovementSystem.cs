...
public sealed class MovementSystem
{
    ...
    {
        var x = Grid.WorldToCell(destination.X.ToIntFloor());
        var y = Grid.WorldToCell(destination.Y.ToIntFloor());
    ...
    private void StepMember(SimWorld world, GameObject member, MemberTarget target)
    {
        ...
        if (!Grid.IsPassable(
            Grid.WorldToCell(position.X.ToIntFloor()),
            Grid.WorldToCell(position.Y.ToIntFloor())))
        {
            var goalX = Grid.WorldToCell(target.Position.X.ToIntFloor());
            var goalY = Grid.WorldToCell(target.Position.Y.ToIntFloor());
    ...
    {
        var goalX = Grid.WorldToCell(destination.X.ToIntFloor());
        var goalY = Grid.WorldToCell(destination.Y.ToIntFloor());
    ...
    {
        var x = field.Grid.WorldToCell(position.X.ToIntFloor());
        var y = field.Grid.WorldToCell(position.Y.ToIntFloor());
    ...
    private static FixedVector2 StepFlow(FlowField field, FixedVector2 position, Fixed64 distance)
    {
        ...
        while (remaining > Fixed64.Zero)
        {
            ...
            if (!field.Grid.IsPassable(
                field.Grid.WorldToCell(candidate.X.ToIntFloor()),
                field.Grid.WorldToCell(candidate.Y.ToIntFloor()))) break;
            position = candidate;
            remaining -= quantum;
        }
        return position;
    ...
    private bool IsInGoalCell(FixedVector2 position, FixedVector2 destination) =>
        Grid.WorldToCell(position.X.ToIntFloor()) == Grid.WorldToCell(destination.X.ToIntFloor())
        && Grid.WorldToCell(position.Y.ToIntFloor()) == Grid.WorldToCell(destination.Y.ToIntFloor());
