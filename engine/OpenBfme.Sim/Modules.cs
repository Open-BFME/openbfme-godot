...
/// buildable entries as "Build:template-name" = build ticks. Finished units spawn
/// at the producer's position offset by ExitOffsetX/Y.
/// </summary>
public sealed class ProductionModule : ModuleBase
{
    ...
    public bool TryQueue(string templateName)
    {
    ...
    public override void OnUpdate(SimWorld world, GameObject self)
    {
        ...
        if (ticksRemaining > 0)
        {
            _queue[0] = (template, ticksRemaining);
            return;
        }
        _queue.RemoveAt(0);
        world.SpawnObject(template, self.Team, self.Position + _exitOffset);
    }
