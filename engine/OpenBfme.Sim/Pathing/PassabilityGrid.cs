...
public sealed class PassabilityGrid
{
    ...
    public int WorldToCell(Fixed64 coordinate) =>
        (coordinate / Fixed64.FromInt(CellSize)).ToIntFloor();

    public (int X, int Y) WorldToCell(FixedVector2 position) =>
        (WorldToCell(position.X), WorldToCell(position.Y));
