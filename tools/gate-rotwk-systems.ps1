...
# When ROTWK_INSTALL (or F:\RotWK) exists: doctor + optional map corpus smoke (limit).
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($RotwkInstall)) {
        foreach ($candidate in @("F:\RotWK", "D:\RotWK", "C:\RotWK")) {
            if (Test-Path -LiteralPath (Join-Path $candidate "game.dat")) {
