...
$focusedRunners = [ordered]@{
    ...
    # per member.
    'combat_sfx_layer' = @('retail_combat_sfx_layer_runner.gd', '(?m)^RETAIL_COMBAT_SFX_LAYER_RESULT passed=23 failed=0\s*$')
    # Owner playtest report C ("the trebuchets are not physically spawned on top
    # of the tower parts"): the authored ObjectCreationUpgrade mount offset, read
    ...
    # RE-PINNED 2026-08-19 (UI PARITY lane): 118 -> 121 — radar draws no shroud
    # darkening (complete disc, REF-52), world-ring button size is the authored
    # 48 stage units, world-ring buttons carry retail tooltips (REF-25).
    'four_unit_hud' = @('retail_four_unit_hud_runner.gd', '(?m)^RETAIL_FOUR_UNIT_HUD_RESULT passed=121 failed=0\s*$')
    ...
    # Pre-Q13 this runner already emitted 98 against a rotten pin of 49.
    # Q13 added 13 projectile/radius checks (98 -> 111). Re-measured 2026-08-17.
    # Q13-fix added 4 damage-component persistence checks (111 -> 115) 2026-08-17.
    'member_combat_runtime' = @('retail_member_combat_runner.gd', '(?m)^RETAIL_MEMBER_COMBAT_RESULT passed=115 failed=0\s*$')
    # Marker is the runner RESULT line. setup() must not seed unarmored kinds or
    # the shared $forbiddenDiagnostics regex trips on Q11 structure-armor ERROR.
