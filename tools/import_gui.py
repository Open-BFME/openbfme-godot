...
def _default_godot() -> Path | None:
    ...
    for name in (
        ...
        candidate = repo_root / ".tools" / "godot" / name
        ...
        def _worker() -> None:
            try:
                report = check_dependencies(
                    ...
                    mode=mode,
                    deep=False,
                    godot_path=GODOT_DEFAULT if GODOT_DEFAULT is not None and GODOT_DEFAULT.is_file() else None,
                )
    ...
    def _launch_godot(self) -> None:
        ...
        if bat.is_file():
            try:
                ...
                pass
        godot = GODOT_DEFAULT
        if godot is not None and godot.is_file():
            subprocess.Popen(
