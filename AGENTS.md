...
**Content pack pruning (Q88).** `workspace/content-packs/` and the durable Godot mirror hold published packs from many builds. `tools/prune-content-packs.ps1` (dry-run by default, `-Execute` to delete) retains selected bundles (active + all supplemental from both selection.json files), one previous digest per pack-id (for rollback), and any digest pinned in `tools/gate-*.ps1` or `game/tests/*runner.gd`. Unselected bundles are deleted. Run post-prune proofs: `check_pack_addresses.py`, `publish-durable-pack.ps1 -Verify`, retail slice boot on selected packs, and gate Section B digest presence. If a d115f938-era unselected pack looks intentionally staged, stop and flag; otherwise prune per owner ruling "we just need 1 good pack".
