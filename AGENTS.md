...
## Standing operations policy

**Content pack pruning (Q88).** `workspace/content-packs/` and the durable Godot mirror hold published packs from many builds. `tools/prune-content-packs.ps1` (dry-run by default, `-Execute` to delete) retains: selected bundles (active + all supplemental from both selection.json files); ALL on-disk digests of every selected pack-id (deliberate over-keep for rollback); and any digest pinned in `tools/gate-*.ps1`, `game/tests/**/*.gd` (recursive), `dist/v0.2.*/content-packs/selection.json`, or `tools/**/*.py` (digest, path-literal, pathlib multi-line, and bare-digest forms). Name-addressed bundle dirs (non-64-hex names) are never deleted without `-AllowNamedBundles`. Empty pin sources are fatal, not permissive. Unselected, unpinned bundles are deleted; `-Execute` writes `workspace/logs/q88-prune/execute.txt` with per-item outcomes. Run post-prune proofs: `check_pack_addresses.py`, `publish-durable-pack.ps1 -Verify`, retail slice boot on selected packs, and gate Section B digest presence. Owner ruling: "we just need 1 good pack". History: the first execution (2026-08-25) deleted two Python-tool-pinned bundles (`bc6ab089…`, `goal-official-72`) because the scan then covered only ps1+gd — that loss is disclosed in `docs/TOPOLOGY.md` and drove the wider scan.
