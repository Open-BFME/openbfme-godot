...
class PrivateRetailPathTests(unittest.TestCase):
    def test_defaults_stay_under_ignored_private_workspace(self) -> None:
        with patch.dict(
            os.environ,
            {"OPENBFME_IMPORT_ROOT": "", "OPENBFME_CONTENT_ROOT": ""},
            clear=False,
        ):
            private_root = (repo_root_from_module() / ".private").resolve()
    ...
    def test_only_private_subtree_is_allowed_inside_repository(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            allowed = repository / ".private" / "retail-work"
