...
def _discover_godot() -> tuple[Path | None, str]:
    ...
    Portable order: env vars -> repo ``.tools/godot/`` -> PATH.
    No maintainer-home paths (e.g. Downloads/godot47).
    """

    for name in ("OPENBFME_GODOT", "GODOT_CONSOLE", "GODOT_EXE", "GODOT"):
    ...
    # importer/openbfme_importer -> repo root is parents[2]
    ...
    for name in (
        ...
        candidate = tools_godot / name
        if candidate.is_file():
            return candidate.resolve(), ".tools/godot"
