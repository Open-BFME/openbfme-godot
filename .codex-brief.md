# Stage 1 — rename `workspace` → `workspace/`, kill the firewall friction
...
The directory `workspace/` is renamed to `workspace/` (internal structure
UNCHANGED — do not reorganize subdirectories), `workspace/retail-extract/` becomes
...
- `git status --porcelain > .private-rename-baseline-status.txt` (temp file, delete before finishing)
...
- `Move-Item workspace workspace`
- `Move-Item workspace/retail-extract workspace\retail-extract`
...
### 3. .gitignore
Replace the `/workspace/` entry and both `workspace/retail-extract` entries with a single
...
276 tracked files reference `workspace`; 5 reference `workspace/retail-extract`.
Mechanically update every tracked-file reference, all variants:
`workspace/`, `workspace\`, `workspace\\`, bare `workspace`, `workspace/retail-extract`.
...
`workspace/retail-work/**` job/profile configs IF they reference `workspace` or
`workspace/retail-extract` (grep inside workspace for both strings, bounded to text/json/
...
1. `git grep -c -F "workspace"` → **zero tracked hits** except `docs/patch-notes/`
   (historical release notes may keep old paths). Same for `workspace/retail-extract`.
...
6. Working tree clean except intended changes; ONE rename commit (after the WIP
   commit), message
   `refactor(layout): rename workspace -> workspace, retire asset-firewall paths`.

### 7. Report
Write `orchestration/reports/stage1-.private-rename.md`: what moved, sweep hit
...
- Python: C:\Users\Jonathan\Desktop\open-bfme\workspace\retail-work\tools\python-3.12-env\Scripts\python.exe with PYTHONPATH pointing at the worktree's importer/.
