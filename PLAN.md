...
- [x] Every retail menu screen is a real, cooked scene - not a hand-built imitation {#shell-screen-cook}
  by: claude
  tech: queue Q117. The screens were never unparsed - they are SCRIPT-DRIVEN state machines, and the Palantir was the exception (348 root placements, 28 on frame 0) rather than the template. Built `retail_screen_apt_plan` (movie-agnostic plan + transitive import closure + atlas resolution) and `retail_screen_apt_convert` (scene contract + atlas PNGs), plus `_Flattener.flatten_screen`, an ADDITIVE sibling that replays a root timeline to an AUTHORED frame while `flatten()` stays byte-for-byte so every Palantir pin holds. Five real parser bugs fell out, each fixed at the root and pinned: atlases were never planned (`"atlases": []` behind a comment claiming the converter resolved them); five zero-operand SWF base-block opcodes were unnamed; an EMPTY function body was read as a truncated one; an authored null clip-action pointer was fatal instead of named; importing one symbol into two character slots was rejected though ZERO movies in the tree duplicate a character id. 84 movies: 83 flatten, 62 cook to contracts, 13,233 primitives, and the only refusal left is a developer test asset. WHICH FRAME is a declared policy (`_open`, `_show`, `_init`, `_fadeIn`) recorded beside every label the movie carries. It also paid back into the shipped shell: `Background` was cooked at frame 0, whose label is literally `_no_BG`, so the main menu shipped WITHOUT its authored backdrop art - now 36 draws / 6 buttons. Godot draws them too: `game/src/ui/retail_screen_apt_runtime.gd` binds a cooked screen by movie name and REFUSES any contract whose frame-selection rule it has not been told about, so a policy change can never quietly present a state nobody chose (runner 25/0, verified failing 2/2 with those checks deleted). The pipeline converter and per-screen profile landed too, so a pack can carry them. Then an owner-requested grok review scored the lane 5/10 and was right: the flagged-null guard was auto-admitting its own hits (now 54 frozen identities, verified both directions), the opcode table cited the SWF spec when this repo already carries the SAGE oracle in `game/src/apt/apt_vm.gd` (re-sourced; found two real gaps), and the frame lock only checked the rule name. A self-audit then caught that the whole lane had been measured against BFME2 rather than the ratified RotWK baseline - 26 of 59 shared screens differ in draw count - so both editions are now pinned with RotWK shipping. Applying the authored-label policy to NESTED sprites too roughly doubled the reconstruction: RotWK 65 screens / 21,410 primitives, and the hero dock and side command bar reconstruct for the first time.
...
## decisions
- 2026-08-28 SCOPE RATIFIED BY THE OWNER, no partial target: "everything in the game needs to be 1:1 with bfme 2 rotwk including every hud, asset, sound, gameplay mechanic everything". That settles the open question of whether to defer the campaigns for cheaper HUD wins - both are in. The complete map is `docs/ONE_TO_ONE_GAP_MAP.md` and it is now FILED AS QUEUE ROWS Q105-Q134 (campaigns, tutorials, shellmap, cinematics, credits, replays, online product, sky/shadows, burning buildings, emissive recook, the 9 uncooked APT screens, ControlBar lanes, palantir residue, the 609-action script vocabulary, RotWK AI, audio depth, naval, spellbook tier, EmotionTracker, ring heroes, WotR handoff, localisation, modding, handicap/stats, the unverified asset census, skirmish map coverage, queue hygiene). grok's criticism was that none of it existed as rows and so was invisible; that is now false. Acceptance stays external: both auditors re-score, and 10/10 needs both to agree.
