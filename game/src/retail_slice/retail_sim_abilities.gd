...
func _apply_ability_activate_module_graph(row: Dictionary, ability_id: String, effect: Dictionary, target_point: Vector2, targeting: String) -> Dictionary:
	...
	var current_target = _sim._activate_module_target_identity(row, target_point, targeting)
	...
	var channel := {
		...
		"start_tick": _sim.tick_index,
		"activation_tick": _sim.tick_index + activation_delay,
		"active_end_tick": _sim.tick_index + activation_delay + duration,
		"finish_tick": _sim.tick_index + activation_delay + duration + pack,
		"dispatched": false,
		"route_results": [],
	}
	...
	row["state"] = "ability"
	_sim._emit_event("ability.graph_started", int(row.get("id", 0)), int(channel.get("current_target_id", 0)), {
...
func _apply_ability_weapon_blast(hero_row: Dictionary, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	var affected := 0
	if radius > 0.0:
		for id in _sim._ability_enemies_near(team, point, radius):
			_sim._apply_damage(attacker_id, id, damage, "battalion")
			affected += 1
		for structure_id in _sim.structure_ids():
			...
			if Vector2(structure.get("position", Vector2.ZERO)).distance_to(point) <= radius:
				_sim._apply_structure_damage(attacker_id, structure_id, damage)
				affected += 1
	else:
		var best_id := -1
		var best_distance := 2.0
		for id in _sim._ability_enemies_near(team, point, 2.0):
			var distance := Vector2((_sim.entities[id] as Dictionary).get("position", Vector2.ZERO)).distance_to(point)
			if distance <= best_distance:
				best_distance = distance
				best_id = id
		if best_id >= 0:
			_sim._apply_damage(attacker_id, best_id, damage, "battalion")
			affected = 1
	...
	if knockback_radius > 0.0 and knockback_strength > 0.0:
		_sim._apply_knockback(point, knockback_radius, knockback_strength, team, 0, "ability-blast", attacker_id)
...
func _apply_ability_heal(hero_row: Dictionary, effect: Dictionary, epicenter: Vector2) -> Dictionary:
	...
	for id in _sim.living_ids(team):
		...
		if not _sim._ability_filter_accepts(row, filter_text):
			continue
	...
	if range_limit > 0.0:
		for id in _sim.living_ids(team):
			...
			if Vector2(ally.get("position", Vector2.ZERO)).distance_to(Vector2(hero_row.get("position", Vector2.ZERO))) <= range_limit:
				targets.append(id)
	var expiry = _sim.tick_index + duration_ticks
	for id in targets:
		_sim._set_timed_modifier(_sim.entities[id] as Dictionary, "ability:%s" % ability_id, modifiers, expiry)
...
func _apply_ability_siege_deploy(row: Dictionary, effect: Dictionary, point: Vector2, contract: Dictionary) -> Dictionary:
	...
	var target_id = _sim._siege_deploy_target(row, effect, point, contract)
	if target_id == 0:
	...
	var lower_ticks = _sim._ship_contract_delay_ticks(float(effect.get("lowerDelayMs", 0)))
	var channel := {
		"special_power_template_id": String(effect.get("specialPowerTemplateId", "")),
		"target_id": target_id,
		"current_target_id": target_id,
		"phase": "lowering",
		"phase_end_tick": _sim.tick_index + lower_ticks,
		"raise_delay_ticks": _sim._ship_contract_delay_ticks(float(effect.get("raiseDelayMs", 0))),
...
func _apply_ability_toggle_deploy(row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	row["toggle_deploy_channel"] = {
		"phase": phase,
		"phase_end_tick": _sim.tick_index + duration_ticks,
		"sound_id": sound_id,
		"modifier_id": String(modifier_leaf.get("id", "")),
		"modifiers": modifiers,
		"presentation_receipt": "model-condition:%s" % phase.to_upper(),
	}
	_sim._toggle_deploy_set_model_condition(row, phase.to_upper())
	_sim._clear_pending_route(row, true)
	_sim._clear_member_targets(row)
	row["target_id"] = 0
	row["target_kind"] = ""
	...
	row["order_kind"] = ""
	row["state"] = "ability"
	_sim._emit_event("ability.toggle_deploy_started", int(row.get("id", 0)), 0, {
		"phase": phase,
...
func _apply_ability_weapon_mode_special_power(row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	if not found:
		...
		var mode = _sim._resolve_weapon_mode_special_power_profile(row, effect.get("weaponSetFlags", []) as Array, String(effect.get("lockWeaponSlot", "")).to_lower())
	...
	return _sim.activate_weapon_mode_special_power(int(row.get("id", 0)), template, int(row.get("team", -1)))
...
func _apply_ability_dominate_enemy(row: Dictionary, ability_id: String, effect: Dictionary, target_point: Vector2, targeting: String) -> Dictionary:
	...
	if not (row.get("dominate_enemy_channel", {}) as Dictionary).is_empty():
		return {"ok": false, "reason": "ability-channel-active"}
	var candidates = _sim._dominate_enemy_candidates(row, effect, target_point, targeting)
	...
	var channel := {
		...
		"start_tick": _sim.tick_index,
		"activation_tick": _sim.tick_index + activation_delay,
		"finish_tick": _sim.tick_index + activation_delay + freeze_ticks,
		"order_sequence_at_start": int(row.get("order_sequence", 0)),
		"triggered": false,
		"affected_ids": [],
	...
	row["current_speed"] = 0.0
	row["state"] = "ability"
	_sim._emit_event("ability.dominate_started", int(row.get("id", 0)), int(candidates[0]), {
...
func _apply_ability_grab_passenger(row: Dictionary, ability_id: String, effect: Dictionary, target_point: Vector2) -> Dictionary:
	...
	if _sim.passenger_count(int(row.get("id", 0))) >= int(containment_rule.get("slots", 0)):
		return {"ok": false, "reason": "capacity-full"}
	var target_id = _sim._grab_passenger_target(row, effect, target_point)
	if target_id <= 0:
	...
	var prep_end = _sim.tick_index + int(timing.get("UnpackTime", 0)) + int(timing.get("PreparationTime", 0))
	...
	row["grab_passenger_channel"] = {
		...
		"start_tick": _sim.tick_index, "preparation_end_tick": prep_end,
	...
	_sim._emit_event("ability.grab_started", int(row.get("id", 0)), target_id, {"ability_id": ability_id, "trigger_tick": trigger_tick, "finish_tick": finish_tick, "animation": animation, "initiate_fx_id": String(effect.get("initiateFxId", ""))})
...
func _apply_ability_repair_structure(row: Dictionary, ability_id: String, effect: Dictionary, target_point: Vector2) -> Dictionary:
	...
	for structure_id in _sim.structure_ids():
		...
		if distance > _sim._structure_footprint_radius(structure) or distance >= best_distance:
			continue
	...
	_sim._emit_event("ability.repair_started", int(row.get("id", 0)), target_id, {"ability_id": ability_id, "fraction_per_second": fraction, "contact_point": "Repair", "resource_cost": null})
...
func _apply_ability_fling_passenger(row: Dictionary, ability_id: String, effect: Dictionary) -> Dictionary:
	...
	if row.has("fling_passenger_channel"):
		return {"ok": false, "reason": "ability-channel-active"}
	var passengers := _sim.containment.get(int(row.get("id", 0)), []) as Array
	...
	row["fling_passenger_channel"] = {
		...
		"start_tick": _sim.tick_index, "trigger_tick": _sim.tick_index + int(timing.get("UnpackTime", 0)),
		"finish_tick": _sim.tick_index + int(timing.get("UnpackTime", 0)) + int(timing.get("PackTime", 0)),
...
func _apply_ability_mount_toggle(hero_row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	if bool(hero_row.get("mounted", false)):
		...
		hero_row.erase("form")
		_sim._rescale_member_health_preserving_fraction(hero_row, int(effect.get("dismountedMemberHealth", 0)))
	...
	if mode != "":
		if not _sim._apply_weapon_mode(hero_row, mode):
	...
	_sim._rescale_member_health_preserving_fraction(hero_row, int(effect.get("mountedMemberHealth", 0)))
...
func _apply_ability_special_disguise(row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	row["special_disguise_channel"] = {
		"phase": "unpacking",
		"phase_start_tick": _sim.tick_index,
		"phase_end_tick": _sim.tick_index + unpack_ticks,
	...
	_sim._emit_special_disguise_presentation(row, "owner-mounted-presentation", String(effect.get("ownerObjectId", "")), true)
	_sim._emit_event("ability.special_disguise_started", int(row.get("id", 0)), 0, {
		"phase": "unpacking", "phase_end_tick": _sim.tick_index + unpack_ticks,
		"presentation_prerequisite_sha256": String(effect.get("presentationPrerequisiteSha256", "")),
	})
...
func _apply_ability_capture_building(hero_row: Dictionary, effect: Dictionary, target_point: Vector2) -> Dictionary:
	...
	for structure_id in _sim.structure_ids():
		...
		if distance > best_distance:
			continue
		if int(structure.get("team", -1)) != _sim.NEUTRAL_TEAM:
	...
	hero_row["capture_channel"] = {
		...
		"complete_tick": _sim.tick_index + channel_ticks,
	}
	...
	_sim._clear_member_attack_schedule(hero_row)
	_sim._clear_member_targets(hero_row)
	_sim._clear_pending_route(hero_row, true)
	hero_row["state"] = "capture"
	_sim._emit_event("structure.capture_started", int(hero_row.get("id", 0)), best_id, {
		"team": hero_team,
		"structure_id": best_id,
...
func _apply_ability_terror(hero_row: Dictionary, ability_id: String, effect: Dictionary) -> Dictionary:
	...
	var expiry = _sim.tick_index + duration_ticks
	var affected := 0
	for id in _sim._ability_enemies_near(team, origin, radius):
		var target: Dictionary = _sim.entities[id]
		if bool(target.get("fear_resistant", false)) or _sim._timed_modifier_active(target, "RESIST_FEAR"):
			continue
		if not _sim._ability_filter_accepts(target, filter_text):
			continue
		_sim._set_timed_modifier(target, "fear:%s" % ability_id, modifiers, expiry)
		affected += 1
		if scatter > 0.0 and not bool(target.get("flying", false)):
			_sim._apply_fear_scatter(origin, target, scatter)
...
func _apply_ability_summon(hero_row: Dictionary, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	for entry_value in effect.get("objects", []) as Array:
		...
		var unit_type = _sim._summon_unit_type_for(source_id)
		if unit_type == "":
			return {"ok": false, "reason": "summon-object-unconverted:%s" % source_id}
		var rule: Dictionary = _sim._unit_production_rules.get(unit_type, {}) as Dictionary
		var member_id := String(rule.get("object_id", ""))
		if member_id == "":
			return {"ok": false, "reason": "summon-object-unconverted:%s" % source_id}
		var count := clampi(int(entry.get("count", 1)), 1, _sim.ABILITY_SUMMON_MAX_COUNT)
		for _index in range(count):
			ordinal += 1
			var new_id := int(_sim._next_dynamic_id.get(team, 0))
			_sim._next_dynamic_id[team] = new_id + 1
			var offset := Vector2(_sim.ABILITY_SUMMON_OFFSET_STEP * float(ordinal), 0.0)
			_sim._add_battalion(new_id, team, point + offset, String(rule.get("display_name", unit_type)), member_id, unit_type)
			if not _sim.entities.has(new_id):
				return {"ok": false, "reason": "summon-spawn-failed:%s" % source_id}
			_sim._emit_event("unit.summoned", new_id, 0, {"team": team, "object_id": member_id, "unit_type": unit_type})
...
func _apply_ability_summon_chain(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	var support = _sim._spellbook_ocl_support(
	...
	var cast = _sim._cast_spellbook_summon(team, resolved, point)
...
func _apply_ability_arrow_storm(hero_row: Dictionary, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	if not can_shoot_empty_ground and _sim._ability_enemies_near(team, point, radius).is_empty():
	...
	hero_row["volley_channel"] = {
		...
		"next_shot_tick": _sim.tick_index + 1,
	}
...
func _apply_ability_stealth_toggle(hero_row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	if _sim._stealth_active(hero_row):
		_sim._clear_stealth(hero_row)
	...
	var until_tick = _sim.STEALTH_UNTIMED_EXPIRY_TICK if untimed else _sim.tick_index + duration_ticks
	_sim._grant_stealth(hero_row, until_tick, forbidden)
	var affected := 1
	...
	if broadcast > 0.0:
		...
		for id in _sim.living_ids(team):
			...
			if not _sim._ability_filter_accepts(ally, filter_text):
				continue
			_sim._grant_stealth(ally, until_tick, forbidden)
			affected += 1
...
func _apply_ability_teleport(hero_row: Dictionary, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	if busy_ticks > 0:
		hero_row["ability_hold_until_tick"] = _sim.tick_index + busy_ticks
	var destination_affected := 0
	if not destination_weapon.is_empty():
		destination_affected = _sim._apply_knockback(
			point,
		...
		if fire_fx_id != "":
			_sim._emit_event("ability.graph_fx", int(hero_row.get("id", 0)), 0, {"fx_id": fire_fx_id, "point": point})
...
func _apply_ability_curse(hero_row: Dictionary, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	var target_row: Dictionary = _sim.entities[best_id]
	...
	for ability_key in ability_keys:
		...
		if cooldown_ticks <= 0:
			continue
		var setback = _sim.tick_index + roundi(float(cooldown_ticks) * minf(percentage, 100.0) / 100.0)
...
func _apply_ability_leadership_strip(hero_row: Dictionary, effect: Dictionary) -> Dictionary:
	...
	var suppression_source := "horn:%d:%d" % [
		int(hero_row.get("id", 0)), _sim.tick_index
	]
	var affected := 0
	for id in _sim._ability_enemies_near(team, origin, radius):
		var target: Dictionary = _sim.entities[id]
		...
		_sim._set_leadership_suppression_source(
			target, suppression_source, _sim.tick_index + duration_ticks
		)
		affected += 1
	if affected == 0:
