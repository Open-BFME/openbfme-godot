...
func _step_structure_weapons() -> void:
	...
	for structure_id_value in sim.structures.keys():
		...
		if not pending.is_empty():
			if sim.tick_index < int(pending.get("impact_tick", sim.tick_index)):
				continue
		...
		if typeof(attack_value) != TYPE_DICTIONARY:
			continue
		if sim.tick_index < int(attack.get("cooldown", 0)):
			continue
		...
		if best_id == 0:
			continue
		var target: Dictionary = sim.entities[best_id]
		...
		if projectile_speed > 0.0 and String(attack.get("projectile_object_id", "")) != "":
			flight_ticks = maxi(1, ceili(best_distance / projectile_speed / sim.TICK_SECONDS))
		...
		attack["pending_projectile"] = {
			...
			"amount": amount,
			"impact_tick": sim.tick_index + int(attack.get("pre_attack_ticks", 0)) + flight_ticks,
		}
		...
		attack["cooldown"] = sim.tick_index + _structure_weapon_period_ticks(attack)
		sim._emit_event("combat.structure_projectile_launched", structure_id, best_id, {
		...
		if int((attack["pending_projectile"] as Dictionary)["impact_tick"]) <= sim.tick_index:
...
func _structure_weapon_period_ticks(attack: Dictionary) -> int:
	...
	if String(attack.get("delay_between_shots_distribution", "")) == "uniform-inclusive-integer":
		...
		if minimum_ms >= 0 and maximum_ms >= minimum_ms:
			var delay_ms = sim.logic_random_int(minimum_ms, maximum_ms)
			return maxi(1, roundi(float(delay_ms) / (sim.TICK_SECONDS * 1000.0)))
...
func _resolve_structure_weapon_impact(
	...
	if not sim.entities.has(target_id):
		sim._emit_event("combat.structure_projectile_cancelled", structure_id, target_id, {
	...
	if member_index >= 0:
		...
		sim.record_hit_reaction(target_id, float(amount))
		...
		if aggregate_health <= 0:
			...
			sim._emit_event("battalion.defeated", structure_id, target_id)
			if bool(death_policy.get("destroy_object", false)):
				sim.entities.erase(target_id)
		else:
			sim._apply_playable_unit_death_policy(target, "NORMAL", defeated_members)
		sim._emit_event("power.structure_weapon_hit", structure_id, target_id, {
			"amount": amount,
			"projectile_token": int(pending.get("token", 0)),
			"projectile_object_id": String(attack.get("projectile_object_id", "")),
		})
...
func _seed_expansion_pads_for(fortress_structure_id: int) -> void:
	if sim.expansion_pads.has(fortress_structure_id) or not sim.structures.has(fortress_structure_id):
		return
	var fortress: Dictionary = sim.structures[fortress_structure_id]
	...
	if not castle.is_empty():
		...
		for pad_kind in sim.EXPANSION_PAD_LAYOUT.keys():
			for slot in range((sim.EXPANSION_PAD_LAYOUT[pad_kind] as Array).size()):
				pads.append({
					"slot": slot,
					"pad_kind": pad_kind,
					"position": center + (sim.EXPANSION_PAD_LAYOUT[pad_kind] as Array)[slot],
					"expansion_structure_id": 0,
				})
	sim.expansion_pads[fortress_structure_id] = pads
...
func _unpack_castle_behavior_for_structure(structure_id: int) -> bool:
	...
	if not sim.structures.has(structure_id):
		return false
	var owner: Dictionary = sim.structures[structure_id]
	...
	if castle.has("_error"):
		...
		sim.configuration_error = message
		push_error(message)
		return false
	...
	for piece_value in castle.get("pieces", []) as Array:
		...
		var at = Vector2(owner.get("position", Vector2.ZERO)) + sim._retail_source_to_sim_offset(offset)
	...
	sim._emit_event("structure.castle_unpacked", 0, structure_id, {
...
func _castle_behavior_for_structure(structure: Dictionary) -> Dictionary:
	...
	var manifest_source := ""
	var manifest_sources: Variant = sim.structure_source_object_ids_for_team(team).get(kind, [])
	...
	for key in [
		...
		if key != "" and sim._castle_behavior_by_source.has(key):
			return (sim._castle_behavior_by_source[key] as Dictionary).duplicate(true)
		# Case-insensitive source map
		for source_key in sim._castle_behavior_by_source.keys():
			if String(source_key).to_lower() == key.to_lower():
				return (sim._castle_behavior_by_source[source_key] as Dictionary).duplicate(true)
	return {}
...
func _spawn_castle_piece_structure(
	...
	sim.structures[structure_id] = {
		...
		"object_id": object_id,
		"position": at,
		"elevation": float(fortress.get("elevation", 0.0)) + sim._retail_source_to_sim_offset(Vector2(elevation_source, 0.0)).x,
...
func _seed_all_expansion_pads() -> void:
	for structure_id in sim.structure_ids():
		if String((sim.structures[structure_id] as Dictionary).get("structure_kind", "")) == "fortress":
...
func _seed_build_plots_for_team(team: int) -> void:
	...
	var center = sim._team_center(team)
	var fortress = sim.fortress_id(team)
	if fortress != 0:
		center = Vector2((sim.structures[fortress] as Dictionary).get("position", center))
	var plots: Array = []
	for offset in sim.BUILD_PLOT_RING_OFFSETS:
		plots.append({"position": center + (offset as Vector2), "occupant_structure_id": 0})
	sim.build_plots[team] = plots
...
func _reconcile_build_plots(team: int) -> void:
	...
	if not sim.build_plots.has(team):
		return
	for plot_value in sim.build_plots[team] as Array:
		var plot: Dictionary = plot_value
		var occupant := int(plot.get("occupant_structure_id", 0))
		if occupant == 0:
			continue
		if not sim.structures.has(occupant) or int((sim.structures[occupant] as Dictionary).get("health", 0)) <= 0:
...
func _free_build_plot_index_near(team: int, position: Vector2) -> int:
	...
	_reconcile_build_plots(team)
	var plots: Array = sim.build_plots.get(team, [])
	var best := -1
	var best_dist = sim.BUILD_PLOT_PICK_RADIUS + 1.0
	for index in plots.size():
		...
		if dist <= sim.BUILD_PLOT_PICK_RADIUS and dist < best_dist:
			best_dist = dist
			best = index
	return best
...
func expansion_commands_for(fortress_structure_id: int) -> Array:
	...
	var result: Array = []
	if not sim.expansion_pads.has(fortress_structure_id):
		return result
	var pads: Array = sim.expansion_pads[fortress_structure_id]
	for kind_value in sim._expansion_build_rules.keys():
		var rule: Dictionary = sim._expansion_build_rules[kind_value]
...
func issue_expansion_construct(team: int, fortress_structure_id: int, expansion_kind: String, requested_pad_index: int = -1) -> Dictionary:
	...
	if not sim.structures.has(fortress_structure_id):
	...
	var rule: Dictionary = sim._expansion_build_rules.get(expansion_kind, {})
	if rule.is_empty():
		return {"ok": false, "reason": "unsupported-expansion"}
	var permission = sim.building_permission_for_kind(team, expansion_kind)
	...
	var structure_id = sim._next_expansion_structure_id
	sim._next_expansion_structure_id += 1
	var maximum_health := int(rule.get("health", 1000))
	var position: Vector2 = (pads[pad_index] as Dictionary).get("position", Vector2.ZERO)
	var build_ticks := maxi(1, roundi(float(rule.get("seconds", 20.0)) / sim.TICK_SECONDS))
	...
	if typeof(attack_value) == TYPE_DICTIONARY and not (attack_value as Dictionary).is_empty():
		...
		attack["cooldown"] = sim.tick_index
		sim.structures[structure_id]["attack"] = attack
		var spawned_object_id := String(attack.get("spawned_object_id", ""))
		if spawned_object_id != "":
			sim.structures[structure_id]["spawned_weapon_object_id"] = spawned_object_id
	...
	sim._emit_event("construction.started", 0, structure_id, {"team": team, "structure_kind": expansion_kind, "cost": cost, "build_ticks": build_ticks})
...
func configure_unpackable_bases(bases: Dictionary) -> bool:
	...
	var reference_teams = sim.script_unit_references.keys()
	reference_teams.sort()
	for name_value in names:
		for team in reference_teams:
			if (sim.script_unit_references[team] as Dictionary).has(String(name_value)):
	...
	if not collisions.is_empty():
		...
		return false
	sim.unpackable_bases = {}
	for name_value in names:
		var spec: Dictionary = bases[name_value]
		sim.unpackable_bases[String(name_value)] = {
...
func base_flag_unpackable(base_name: String) -> Dictionary:
	...
	if not sim.unpackable_bases.has(base_name):
		return {"known": false}
	var row: Dictionary = sim.unpackable_bases[base_name]
	return {
		"known": true,
		"unpackable": sim.base_loop_enabled and sim.winner == -1 and int(row.get("unpacked_by", -1)) < 0,
	}
...
func unpack_base(team: int, base_name: String, free: bool) -> Dictionary:
	...
	if not sim._roster_team_ids().has(team):
	...
	var fortress_build_rule: Dictionary = (
		sim.structure_build_rules_for_team(team).get("fortress", {}) as Dictionary
	)
	...
	var fortress_sources: Dictionary = sim.structure_source_object_ids_for_team(team)
	var fortress_aliases: Array = fortress_sources.get("fortress", []) as Array
	if not fortress_aliases.is_empty():
		sim.structures[structure_id]["source_object_id"] = String(fortress_aliases[0])
	...
	sim._emit_event("base.unpacked", 0, structure_id, {"team": team, "base": base_name, "cost": cost, "free": free})
