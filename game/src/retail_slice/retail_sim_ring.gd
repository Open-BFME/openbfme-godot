...
func _configure_ring_mechanic_contract() -> void:
	...
	if String(sim._ring_contract.get("schema", "")) == "openbfme.ring-system-runtime":
		var compiled_contract = _compiled_ring_runtime_contract(sim._ring_contract)
		if compiled_contract.is_empty():
			sim.configuration_error = "Compiled ring-system runtime registration is invalid"
		else:
			sim._ring_contract = compiled_contract
	for registry_key in ["ring_runtime_documents", "playable_unit_runtimes"]:
		var registry: Variant = sim._rules.get(registry_key, {})
		...
		for document_value in (registry as Dictionary).values():
			...
			for block_name in ["gollum", "ring"]:
				...
				if typeof(block) == TYPE_DICTIONARY:
					sim._ring_contract.merge(block as Dictionary, false)
					if not (block as Dictionary).is_empty():
						...
						if document_object_id != "":
							sim._ring_contract["gollumObjectId" if block_name == "gollum" else "ringObjectId"] = document_object_id
						if block_name == "ring" and (block as Dictionary).has("scanRange"):
							sim._ring_contract["pickupRange"] = float((block as Dictionary)["scanRange"])
	sim._ring_delivery_kinds.clear()
	for kind_value in (sim._rules.get("ring_delivery_structure_kinds", []) as Array):
		sim._ring_delivery_kinds[String(kind_value)] = true
	var producer_kinds: Dictionary = sim._rules.get("producer_kind_by_source_object", {}) as Dictionary
	var structure_runtimes: Variant = sim._rules.get("playable_structure_runtimes", {})
	if typeof(structure_runtimes) == TYPE_DICTIONARY:
		for document_value in (structure_runtimes as Dictionary).values():
			...
			if kind != "":
				sim._ring_delivery_kinds[kind] = (delivery as Dictionary).duplicate(true)
	if sim.ring_mechanic_enabled and sim._ring_contract.is_empty():
		sim._ring_contract = {
			"waypointFamily": "SpawnPoint_SkirmishGollum_", "spawnTeam": sim.CREEP_TEAM,
			"modeToken": "ringheroes", "gollumObjectId": sim.RING_DEFAULT_GOLLUM,
			"ringObjectId": sim.RING_DEFAULT_ITEM, "pickupRange": 10.0,
...
func _ring_state() -> Dictionary:
	if not sim.script_surface_bag.has("ring_mechanic") \
			or typeof(sim.script_surface_bag["ring_mechanic"]) != TYPE_DICTIONARY:
		sim.script_surface_bag["ring_mechanic"] = {
			"gollum_id": 0, "gollum_spawned": false, "ring_active": false,
			"ring_position": Vector2.ZERO, "carrier_id": 0,
		}
	return sim.script_surface_bag["ring_mechanic"] as Dictionary


func _mark_ring_delivery_structures() -> void:
	for structure_id in sim.structure_ids():
		_mark_ring_delivery_structure(sim.structures[structure_id] as Dictionary)
...
func _mark_ring_delivery_structure(row: Dictionary) -> void:
	...
	if sim._ring_delivery_kinds.has(kind):
		row["ring_delivery"] = (sim._ring_delivery_kinds[kind] as Dictionary).duplicate(true) \
			if typeof(sim._ring_delivery_kinds[kind]) == TYPE_DICTIONARY else {}


func _ring_gollum_object_id() -> String:
	return String(sim._ring_contract.get("gollumObjectId", sim.RING_DEFAULT_GOLLUM))


func _ring_spawn_team() -> int:
	var value: Variant = sim._ring_contract.get("spawnTeam", sim.CREEP_TEAM)
	return int(value) if typeof(value) in [TYPE_INT, TYPE_FLOAT] else sim.CREEP_TEAM
...
func ring_presentation_contract() -> Dictionary:
	var offset_value: Variant = sim._ring_contract.get("carrierOffsetSource", Vector2(0.0, -10.0))
	...
	return {
		"enabled": sim.ring_mechanic_enabled,
		"object_id": String(sim._ring_contract.get("ringObjectId", sim.RING_DEFAULT_ITEM)),
		"status": String(sim._ring_contract.get("status", "HOLDING_THE_RING")),
...
func _spawn_ring_gollum_fallback() -> void:
	...
	if existing != 0:
		...
		return
	var family = String(sim._ring_contract.get("waypointFamily", "SpawnPoint_SkirmishGollum_"))
	var rolled = sim.logic_random_int(1, 8)
	var waypoint := "%s%d" % [family, rolled]
	var at: Vector2
	if sim.script_waypoints.has(waypoint):
		at = Vector2(sim.script_waypoints[waypoint])
	else:
		var candidates: Array[String] = []
		for name_value in sim.script_waypoints.keys():
		...
		if not candidates.is_empty():
			...
			at = Vector2.ZERO
	push_warning("RING_GOLLUM_FALLBACK scripted spawn absent after tick %d; deterministic fallback waypoint=%s roll=%d" % [sim.RING_FALLBACK_TICK, waypoint, rolled])
	var gollum_id = sim.spawn_script_object(
	...
	state["gollum_spawned"] = true
	_configure_ring_gollum(sim.entities[gollum_id] as Dictionary)
	sim._emit_event("ring.gollum_spawned", gollum_id, 0, {"waypoint": waypoint, "fallback": true})
...
func _configure_ring_gollum(row: Dictionary) -> void:
	...
	row["ring_wander_percentage"] = int(sim._ring_contract.get("wanderPercentage", 80))
	row["ring_detection_range"] = float(sim._ring_contract.get("detectionRange", 120.0)) * source_scale
	row["ring_flee_enemy_range"] = float(sim._ring_contract.get("fleeEnemyRange", 300.0)) * source_scale
	row["ring_flee_distance"] = float(sim._ring_contract.get("fleeDistance", 800.0)) * source_scale
	if not sim._stealth_active(row):
		sim._grant_stealth(row, 0x3FFFFFFF, [])


func _drop_ring(at: Vector2, source_id: int, reason: String) -> void:
	var state := _ring_state()
	var old_carrier := int(state.get("carrier_id", 0))
	if old_carrier != 0 and sim.entities.has(old_carrier):
		sim.set_entity_object_status(old_carrier, String(sim._ring_contract.get("status", "HOLDING_THE_RING")), false)
	state["ring_active"] = true
	state["ring_position"] = at
	state["carrier_id"] = 0
	sim._emit_event("ring.dropped", source_id, 0, {"position": at, "reason": reason, "object_id": String(sim._ring_contract.get("ringObjectId", sim.RING_DEFAULT_ITEM)), "eva": _ring_eva("dropped")})
...
func _step_ring_gollum(gollum_id: int) -> void:
	if not sim.entities.has(gollum_id):
		return
	var row: Dictionary = sim.entities[gollum_id]
	...
	var detector = sim._spatial_nearest_hostile(row, gollum_team, position, detect_range, 0, true)
	if detector != 0:
		if sim._stealth_active(row):
			sim._clear_stealth(row)
	else:
		if not sim._stealth_active(row):
			sim._grant_stealth(row, 0x3FFFFFFF, [])
	var flee_range := float(row.get("ring_flee_enemy_range", 300.0))
	var threat = sim._spatial_nearest_hostile(row, gollum_team, position, flee_range, 0, true)
	if threat != 0:
		var away = Vector2((sim.entities[threat] as Dictionary).get("position", position)).direction_to(position)
		...
		if sim._assign_route(row, destination):
			...
			sim._emit_event("ring.gollum_flee", gollum_id, threat, {"destination": destination})
		return
	if not (row.get("route", []) as Array).is_empty() or (sim.tick_index + gollum_id) % 20 != 0:
		return
	if sim.logic_random_int(1, 100) > int(row.get("ring_wander_percentage", 80)):
		return
	var angle = TAU * float(sim.logic_random_int(0, 359)) / 360.0
	var distance = float(sim.logic_random_int(8, 24))
	if sim._assign_route(row, position + Vector2.RIGHT.rotated(angle) * distance):
		row["state"] = "run"
		row["ring_wander_count"] = int(row.get("ring_wander_count", 0)) + 1
		sim._emit_event("ring.gollum_wander", gollum_id, 0, {"destination": row.get("destination", position)})
...
func _step_ring_mechanic() -> void:
	...
	if not bool(state.get("gollum_spawned", false)):
		for entity_id in sim.entity_ids():
			if _is_ring_gollum(sim.entities[entity_id] as Dictionary):
				state["gollum_id"] = entity_id
				state["gollum_spawned"] = true
				_configure_ring_gollum(sim.entities[entity_id] as Dictionary)
				sim._emit_event("ring.gollum_spawned", entity_id, 0, {"fallback": false})
				break
	if not bool(state.get("gollum_spawned", false)) and sim.tick_index >= sim.RING_FALLBACK_TICK:
	...
	if carrier_id != 0 and sim.entities.has(carrier_id) and int((sim.entities[carrier_id] as Dictionary).get("health", 0)) > 0:
		var carrier: Dictionary = sim.entities[carrier_id]
		state["ring_position"] = Vector2(carrier.get("position", Vector2.ZERO))
		sim._ensure_parity()
		for team in sim._roster_team_ids():
			sim.parity.fog_reveal(int(team), state["ring_position"], 1.0, true)
		for structure_id in sim.structure_ids(int(carrier.get("team", -1))):
			...
			var range = float(delivery.get("scanRange", sim._ring_contract.get("deliveryRange", 10.0))) * maxf(0.000001, float(sim._rules.get("source_map_transform_scale", 1.0)))
			if Vector2(structure.get("position", Vector2.ZERO)).distance_to(Vector2(carrier.get("position", Vector2.ZERO))) <= range:
				...
				sim.set_entity_object_status(carrier_id, String(sim._ring_contract.get("status", "HOLDING_THE_RING")), false)
				state["ring_active"] = false
				state["carrier_id"] = 0
				sim._emit_event("ring.delivered", carrier_id, structure_id, {"team": team, "eva": _ring_eva("delivered")})
				return
	...
	var pickup_range = float(sim._ring_contract.get("pickupRange", 10.0)) * maxf(0.000001, float(sim._rules.get("source_map_transform_scale", 1.0)))
	for entity_id in sim.entity_ids():
		var candidate: Dictionary = sim.entities[entity_id]
		...
		sim.set_entity_object_status(entity_id, String(sim._ring_contract.get("status", "HOLDING_THE_RING")), true)
		sim._emit_event("ring.picked_up", entity_id, 0, {"position": ring_position, "eva": _ring_eva("pickedUp")})
		sim._emit_event("ring.carrier_revealed", entity_id, 0, {"teams": sim._roster_team_ids()})
		break
