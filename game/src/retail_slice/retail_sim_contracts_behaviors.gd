...
func _attach_castle_member_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var counts_value: Variant = sim._module_contract_value(fields, "CountsForEvaCastleBreached", true)
	var store_value: Variant = sim._module_contract_value(fields, "StoreUpgradePrice", false)
	...
	for key in ["BeingBuiltSound", "CampDestroyedOwnerEvaEvent", "CampDestroyedAllyEvaEvent", "CampDestroyedAttackerEvaEvent"]:
		var value = String(sim._module_contract_value(fields, key, "")).strip_edges()
		if value != "":
...
func _attach_flammable_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for bool_key in ["BurnContained", "SetBurnedStatus"]:
		if fields.has(bool_key) and typeof(sim._module_contract_value(fields, bool_key, null)) != TYPE_BOOL:
			return
	...
	var policy := {
		...
		"next_damage_tick": -1,
		"burned_tick": -1,
		"burn_contained": bool(sim._module_contract_value(fields, "BurnContained", false)),
		"set_burned_status": bool(sim._module_contract_value(fields, "SetBurnedStatus", false)),
		"damage_type": String(sim._module_contract_value(fields, "DamageType", "")),
		"fire_fx": (fields.get("FireFXList", []) as Array).duplicate(true),
		"burning_sound_id": String(sim._module_contract_value(fields, "BurningSoundName", "")),
		"tag": String(contract.get("tag", "")),
		"line": int(contract.get("line", 0)),
	...
	for field_name_value in numeric_fields:
		...
		if not fields.has(field_name):
			continue
		var value: Variant = sim._module_contract_value(fields, field_name, null)
		...
		if field_name in ["AflameDuration", "AflameDamageDelay", "FlameDamageExpiration", "BurnedDelay"]:
			policy[target_key] = sim._ship_contract_delay_ticks(float(value))
		else:
...
func _attach_dynamic_portal_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var filter_value: Variant = sim._module_contract_value(fields, "ObjectFilter", [])
	var prefix_value: Variant = sim._module_contract_value(fields, "BonePrefix", "")
	var bone_count_value: Variant = sim._module_contract_value(fields, "NumberOfBones", 0)
	...
	if typeof(delay_value) in [TYPE_INT, TYPE_FLOAT] and float(delay_value) >= 0.0:
		delay_ticks = sim._ship_contract_delay_ticks(float(delay_value))
	elif fields.has("ActivationDelaySeconds"):
		unsupported.append("unresolved-expression:ActivationDelaySeconds")
	var generated = bool(sim._module_contract_value(fields, "GenerateNow", false))
	var triggered_by = String(sim._module_contract_value(fields, "TriggeredBy", ""))
	...
	row["dynamic_portal"] = {
		...
		"conflicts_with": Array(sim._module_contract_value(fields, "ConflictsWith", [])).duplicate(),
		"allow_enemies": bool(sim._module_contract_value(fields, "AllowEnemies", false)),
		"object_filter": (filter_value as Array).duplicate(),
		"bone_prefix": String(prefix_value),
		"number_of_bones": int(bone_count_value),
		"waypoints": waypoints,
		"links": links,
		"top_attack_position_source": sim._module_contract_value(fields, "TopAttackPos", {}),
		"top_attack_radius_source": sim._module_contract_value(fields, "TopAttackRadius", null),
...
func request_dynamic_portal_route(portal_id: int, entity_id: int, from_index: int, to_index: int) -> Dictionary:
	...
	if not portal_row.has("dynamic_portal"): sim._attach_structure_module_contracts(portal_row)
...
func _attach_wall_hub_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for required in ["Options", "MaxBuildoutDistance", "EffectiveMaxBuildoutDistance", "SegmentTemplateName", "HubCapTemplateName", "DefaultSegmentTemplateName"]:
		if not fields.has(required): return
	var option = String(sim._module_contract_value(fields, "Options", ""))
	...
	var builder_source = float(sim._module_contract_value(fields, "BuilderRadius", 0.0))
	...
	if fields.has("StaggeredBuildFactor"): unsupported.append("staggered-build-factor-engine-define:%s" % String(sim._module_contract_value(fields, "StaggeredBuildFactor", "")))
...
func request_wall_hub_plan(structure_id: int, option: String, endpoint: Vector2) -> Dictionary:
	...
	var row := sim.structures[structure_id] as Dictionary
	if not row.has("wall_hub_behaviors"): sim._attach_structure_module_contracts(row)
...
func _attach_attach_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields := contract.get("fields", {}) as Dictionary
	var filter_value: Variant = sim._module_contract_value(fields, "ObjectFilter", null)
	...
	var scan_source = float(sim._module_contract_value(fields, "ScanRange", 0.0))
	if not is_finite(scan_source) or scan_source < 0.0: return
	var parent_status: Array[String] = []
	for value in sim._typed_contract_tokens(fields, "ParentStatus"): parent_status.append(String(value))
	var unsupported: Array[String] = []
	if bool(sim._module_contract_value(fields,"AnchorToTopOfGeometry",false)): unsupported.append("model-geometry-top-transform-unresolved")
	...
	row["attach_update"] = {"object_filter":object_filter,"scan_range_authored":scan_authored,"scan_range_source":scan_source,"scan_range":sim._retail_source_to_sim_offset(Vector2(scan_source,0.0)).x,"parent_status":parent_status,"always_teleport":bool(sim._module_contract_value(fields,"AlwaysTeleport",false)),"anchor_to_top":bool(sim._module_contract_value(fields,"AnchorToTopOfGeometry",false)),"owner_attach_eva":String(sim._module_contract_value(fields,"ParentOwnerAttachmentEvaEvent","")),"enemy_attach_eva":String(sim._module_contract_value(fields,"ParentEnemyAttachmentEvaEvent","")),"parent_died_eva":String(sim._module_contract_value(fields,"ParentOwnerDiedEvaEvent","")),"unsupported_semantics":unsupported,"source_ini":String(contract.get("sourceIni","")),"tag":String(contract.get("tag","")),"line":int(contract.get("line",0))}
...
func request_attach_update(child_id: int, parent_id: int, parent_kind: String = "entity") -> Dictionary:
	...
	if not child.has("attach_update"): sim._attach_module_contracts(child)
...
func set_entity_upgrade_state(entity_id: int, upgrade_id: String, installed: bool) -> Dictionary:
	...
	sim._reconcile_attribute_modifier_upgrades(row)
	sim._reconcile_geometry_upgrades(row)
...
func _attach_replace_self_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed":
		return
	var fields := contract.get("fields", {}) as Dictionary
	var replace_with = String(sim._module_contract_value(fields, "ReplaceWith", "")).strip_edges()
	var triggered_by = String(sim._module_contract_value(fields, "TriggeredBy", "")).strip_edges()
	var conflicts: Array[String] = []
	var conflict_value: Variant = sim._module_contract_value(fields, "ConflictsWith", [])
...
func _apply_due_replace_self_policy(structure_id: int, row: Dictionary, trigger_upgrade_id: String) -> Dictionary:
	for policy_value in row.get("replace_self_upgrades", []) as Array:
		...
		sim._attach_structure_module_contracts(replacement)
		...
		for index in addition_specs.size():
			...
			sim._attach_structure_module_contracts(addition)
...
func _attach_citadel_slaughter_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("citadel_slaughter"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	var passenger_filter = sim._typed_contract_tokens(fields, "PassengerFilter")
	...
	var capacity: Variant = sim._module_contract_value(fields, "ContainMax", null)
	...
	var upgrade_value: Variant = sim._module_contract_value(fields, "UpgradeForRingEntry", [])
	if typeof(upgrade_value) != TYPE_ARRAY:
		return
	for value in upgrade_value as Array:
		upgrades.append(String(value))
	var destroy_filter = sim._typed_contract_tokens(fields, "ObjectToDestroyForRingEntry")
	...
	row["citadel_slaughter"] = {
		...
		"ring_status": String(sim._module_contract_value(fields, "StatusForRingEntry", "")),
		"ring_upgrades": upgrades,
		"ring_destroy_filter": destroy_filter,
		"ring_fx": String(sim._module_contract_value(fields, "FXForRingEntry", "")),
		"slaughter_count": 0,
		"cashback_total": 0,
		"ring_entry_count": 0,
...
func enter_citadel_slaughter(structure_id: int, entity_id: int) -> Dictionary:
	...
	if not citadel.has("citadel_slaughter"):
		sim._attach_structure_module_contracts(citadel)
...
func _attach_ocl_update_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var fields=contract.get("fields",{}) as Dictionary;var minimum=sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"MinDelay",0.0)));var maximum=sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"MaxDelay",0.0)))
	row["ocl_update"]={"ocl":String(sim._module_contract_value(fields,"OCL","")),"minimum_ticks":minimum,"maximum_ticks":maximum,"amount":int(sim._module_contract_value(fields,"Amount",1)),"next_tick":sim.tick_index+sim.logic_random_int(minimum,maximum),"emission_count":0,"unsupported_semantics":[]}
...
func _attach_stances_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("stances_behavior"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	var template = String(sim._module_contract_value(fields, "StanceTemplate", "")).strip_edges()
	if template == "":
		return
...
func _attach_attribute_modifier_aura_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for existing_value in row.get("attribute_modifier_auras", []) as Array:
		...
		if String(existing.get("tag", "")) == String(contract.get("tag", "")) and int(existing.get("line", -1)) == int(contract.get("line", -2)):
			return
	var fields := contract.get("fields", {}) as Dictionary
	var starts: Variant = sim._module_contract_value(fields, "StartsActive", null)
	var bonus = String(sim._module_contract_value(fields, "BonusName", "")).strip_edges()
	...
	rows.append({
		...
		"triggered_by": sim._typed_contract_tokens(fields, "TriggeredBy"),
		"conflicts_with": sim._typed_contract_tokens(fields, "ConflictsWith"),
		"object_filter": sim._typed_contract_tokens(fields, "ObjectFilter"),
		"anti_categories": sim._typed_contract_tokens(fields, "AntiCategory"),
		"required_conditions": sim._typed_contract_tokens(fields, "RequiredConditions"),
		"target_enemy": bool(sim._module_contract_value(fields, "TargetEnemy", false)),
		"allow_self": bool(sim._module_contract_value(fields, "AllowSelf", false)),
		"run_while_dead": bool(sim._module_contract_value(fields, "RunWhileDead", false)),
		"affect_contained_only": bool(sim._module_contract_value(fields, "AffectContainedOnly", false)),
		"max_active_rank": int(sim._module_contract_value(fields, "MaxActiveRank", 2147483647)),
		"range_source": range_source,
		"refresh_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "RefreshDelay", 0.0)))),
...
func _attach_lifetime_update_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("lifetime_update"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	var wait_for_wakeup = bool(sim._module_contract_value(fields, "WaitForWakeUp", false))
	...
	row["lifetime_update"] = {
		...
		"expire_tick": -1,
		"death_type": String(sim._module_contract_value(fields, "DeathType", "NORMAL")).to_upper(),
		"unsupported_semantics": unsupported,
		"tag": String(contract.get("tag", "")),
		"line": int(contract.get("line", 0)),
	}
...
func _arm_lifetime(row: Dictionary) -> bool:
	...
	lifetime["expire_tick"] = sim.tick_index + sim._ship_contract_delay_ticks(float(duration_ms))
...
func _expire_lifetime_entity(entity_id: int, row: Dictionary, death_type: String) -> void:
	...
	row["health"] = 0
	sim._schedule_respawn_update(entity_id, row, death_type, 0)
...
func _apply_typed_aura_to_target(source_id: int, target: Dictionary, rule: Dictionary, modifier: Dictionary, refresh_ticks: int) -> void:
	...
	if bool(stacking.get("ignoreIfAnticategoryActive", false)) and (rule.get("anti_categories", []) as Array).has(category):
		return
	var duration_ticks = maxi(1, sim._ship_contract_delay_ticks(float(modifier.get("duration_ms", refresh_ticks * sim.TICK_SECONDS * 1000.0))))
...
func _attach_random_sound_selector(row: Dictionary, contract: Dictionary) -> void:
	...
	row["random_sound_selector"] = {"chance_fraction": float(ratio), "reroll_every_frame": bool(sim._module_contract_value(fields, "RerollOnEveryFrame", false)), "voice_priority": int(sim._module_contract_value(fields, "VoicePriority", 0)), "unsupported_semantics": ["reroll_every_frame_requires_presentation_frame_clock"] if bool(sim._module_contract_value(fields, "RerollOnEveryFrame", false)) else [], "rng_receipt": "presentation_sequence_hash_not_gameplay_rng"}
...
func _attach_large_group_audio_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields = contract.get("fields", {}) as Dictionary; var key = sim._typed_contract_tokens(fields, "Key")
	if key.is_empty(): return
	row["large_group_audio"] = {"category_key": key, "unit_weight": int(sim._module_contract_value(fields, "UnitWeight", 1)), "rng_receipt": "presentation_sequence_hash_not_gameplay_rng"}
...
func emit_typed_audio_intent(entity_id: int, sound_role: String) -> Dictionary:
	...
	var row := sim.entities[entity_id] as Dictionary
	if not row.has("model_condition_sound_selectors"): sim._attach_module_contracts(row)
	...
	for selector_value in row.get("model_condition_sound_selectors", []) as Array:
		...
		for state_value in (selector_value as Dictionary).get("states", []) as Array:
			...
			var priority = int(sim._module_contract_value(sounds, "VoicePriority", 0)); candidates.append({"event_id": String((sound as Dictionary).get("value", "")), "priority": priority, "line": int(state.get("line", 0))})
...
func emit_large_group_audio_intent(entity_ids_value: Array, sound_role: String) -> Dictionary:
	...
	for value in entity_ids_value:
		...
		var row = sim.entities[entity_id] as Dictionary; if not row.has("large_group_audio"): sim._attach_module_contracts(row)
...
func _attach_fire_spread_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("fire_spread_update"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	var minimum = sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "MinSpreadDelay", 0.0)))
	var maximum = sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "MaxSpreadDelay", 0.0)))
	var spread_range = float(sim._module_contract_value(fields, "SpreadTryRange", -1.0))
...
func _attach_radiate_fear_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if radius_source < 0.0:
		return
	var fear_type = "UNCONTROLLABLE" if bool(sim._module_contract_value(fields, "GenerateUncontrollableFear", false)) else ("TERROR" if bool(sim._module_contract_value(fields, "GenerateTerror", false)) else "FEAR")
	row["radiate_fear"] = {"active": bool(sim._module_contract_value(fields, "InitiallyActive", false)), "triggered_by": String(sim._module_contract_value(fields, "TriggeredBy", "")), "special_power": int(sim._module_contract_value(fields, "WhichSpecialPower", -1)), "fear_type": fear_type, "radius_source": radius_source, "interval_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "EmotionPulseInterval", 0.0)))), "next_tick": sim.tick_index, "victim_filter": sim._typed_contract_tokens(fields, "VictimFilter"), "pulse_count": 0, "unsupported_semantics": unsupported, "tag": String(contract.get("tag", "")), "line": int(contract.get("line", 0))}
...
func _attach_poisoned_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields := contract.get("fields", {}) as Dictionary
	row["poisoned_behavior"] = {"interval_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "PoisonDamageInterval", 0.0)))), "duration_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "PoisonDuration", 0.0)))), "active": false, "damage_per_pulse": 0.0, "next_tick": -1, "expires_tick": -1, "pulse_count": 0, "unsupported_semantics": []}
...
func _attach_damage_field_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var weapon = String(sim._module_contract_value(nugget, "WeaponName", "")); var unsupported: Array[String] = []
	if not sim._death_weapon_rules.has(weapon): unsupported.append("unresolved_damage_field_weapon:%s" % weapon)
	for token in sim._typed_contract_tokens(fields, "ObjectFilter"):
		if token not in ["ALL", "NONE", "ENEMIES", "ALLIES"]:
			unsupported.append("unsupported_damage_field_filter_token:%s" % token)
	row["damage_field"] = {"radius_source": float(sim._module_contract_value(fields, "Radius", 0.0)), "object_filter": sim._typed_contract_tokens(fields, "ObjectFilter"), "required_upgrade": String(sim._module_contract_value(fields, "RequiredUpgrade", "")), "weapon": weapon, "delay_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(nugget, "FireDelay", 0.0)))), "one_shot": bool(sim._module_contract_value(nugget, "OneShot", false)), "next_tick": sim.tick_index + maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(nugget, "FireDelay", 0.0)))), "fired": false, "unsupported_semantics": unsupported}
...
func _attach_spawn_unit_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields := contract.get("fields", {}) as Dictionary
	row["spawn_unit_behavior"] = {"unit_name": String(sim._module_contract_value(fields, "UnitName", "")), "unit_command": String(sim._module_contract_value(fields, "UnitCommand", "")), "spawn_once": bool(sim._module_contract_value(fields, "SpawnOnce", false)), "spawned": false, "spawned_ids": [], "pending": not fields.has("UnitCommand"), "unsupported_semantics": []}
...
func _attach_hit_reaction_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for index in range(1, 4):
		...
		if not fields.has(timer_key):
			continue
		var timer: Variant = sim._module_contract_value(fields, timer_key, null)
		var threshold: Variant = sim._module_contract_value(fields, threshold_key, null)
		if typeof(timer) != TYPE_INT or typeof(threshold) not in [TYPE_INT, TYPE_FLOAT]:
			return
		tiers.append({"tier": index, "life_ticks": sim._ship_contract_delay_ticks(float(timer)), "threshold": float(threshold)})
	if tiers.size() not in [1, 3]:
		return
	row["hit_reaction"] = {"tiers": tiers, "fast_hits_reset": bool(sim._module_contract_value(fields, "FastHitsResetReaction", false)), "hits": [], "active_tier": 0, "expires_tick": -1, "unsupported_semantics": ["reaction_animation_requires_presentation_binding"], "tag": String(contract.get("tag", "")), "line": int(contract.get("line", 0))}
...
func _attach_animal_ai_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("animal_ai"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	row["animal_ai"] = {"flee_range_source": float(sim._module_contract_value(fields, "FleeRange", 0.0)), "flee_distance_source": float(sim._module_contract_value(fields, "FleeDistance", sim._module_contract_value(fields, "MaxWanderDistance", 0.0))), "wander_percent": float(sim._module_contract_value(fields, "WanderPercentage", 0.0)), "max_wander_distance_source": float(sim._module_contract_value(fields, "MaxWanderDistance", 0.0)), "max_wander_radius_source": float(sim._module_contract_value(fields, "MaxWanderRadius", 0.0)), "update_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "UpdateTimer", 1000.0)))), "next_tick": sim.tick_index, "home": Vector2(row.get("position", Vector2.ZERO)), "unsupported_semantics": [], "tag": String(contract.get("tag", "")), "line": int(contract.get("line", 0))}
...
func _attach_threat_finder_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("threat_finder"):
		return
	var radius: Variant = sim._module_contract_value(contract.get("fields", {}) as Dictionary, "DefaultRadius", null)
...
func _attach_large_group_bonus_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	row["large_group_bonus"] = {
		"update_ticks": maxi(1, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "UpdateRate", 0.0)))),
		"next_tick": sim.tick_index,
		"member_filter": sim._typed_contract_tokens(fields, "HordeMemberFilter"),
		"count": int(sim._module_contract_value(fields, "Count", 1)),
		"radius_source": float(sim._module_contract_value(fields, "Radius", 0.0)),
		"ruboff_radius_source": float(sim._module_contract_value(fields, "RubOffRadius", 0.0)),
		"allies_only": bool(sim._module_contract_value(fields, "AlliesOnly", false)),
...
func _step_large_group_bonus_updates() -> void:
	for source_id in sim.entity_ids():
		...
		if not source.has("large_group_bonus"):
			sim._attach_module_contracts(source)
