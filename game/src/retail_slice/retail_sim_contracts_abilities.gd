...
func _attach_pickup_stuff_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	row["pickup_stuff_update"] = {
		"skirmish_ai_only": bool(sim._module_contract_value(fields, "SkirmishAIOnly", false)),
		"filter": sim._typed_contract_tokens(fields, "StuffToPickUp"),
		"scan_range_source": maxf(0.0, float(sim._module_contract_value(fields, "ScanRange", 0.0))),
		"scan_interval_ticks": maxi(1, sim._ship_contract_delay_ticks(interval_ms)),
...
func _attach_auto_ability_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	rows.append({
		"key": key,
		"special_ability": String(sim._module_contract_value(fields, "SpecialAbility", "")),
		"active": bool(sim._module_contract_value(fields, "StartsActive", false)),
		"base_max_range_from_start": bool(sim._module_contract_value(fields, "BaseMaxRangeFromStartPos", false)),
		"adjust_melee_position": bool(sim._module_contract_value(fields, "AdjustAttackMeleePosition", false)),
		"allow_self": bool(sim._module_contract_value(fields, "AllowSelf", false)),
		...
		"idle_ticks": sim._ship_contract_delay_ticks(idle_ms),
		"forbidden_status": sim._typed_contract_tokens(fields, "ForbiddenStatus"),
		"queries": (fields.get("Query", []) as Array).duplicate(true),
		"next_check_tick": sim.tick_index,
		"unsupported_semantics": ["melee_position_solver:AdjustAttackMeleePosition"] if bool(sim._module_contract_value(fields, "AdjustAttackMeleePosition", false)) else [],
	})
...
func _attach_ai_special_power_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed":
		return
	var fields := contract.get("fields", {}) as Dictionary
	var command = String(sim._module_contract_value(fields, "CommandButtonName", "")).strip_edges()
	var ai_type = String(sim._module_contract_value(fields, "SpecialPowerAIType", "")).strip_edges().to_upper()
	...
	policies.append({
		...
		"spell_makes_structure": bool(sim._module_contract_value(fields, "SpellMakesAStructure", false)),
		"randomize_target_location": bool(sim._module_contract_value(fields, "RandomizeTargetLocation", false)),
	...
	for entity_id in sim.entity_ids():
		...
		for index in policies.size():
			...
			if stance != "":
				...
				var stance_result = sim.set_entity_stance(entity_id, stance)
...
func _attach_respawn_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	row["respawn_update"] = {
		...
		"time_ticks": sim._ship_contract_delay_ticks(float(rules.get("timeMilliseconds", 0.0))),
		"health_fraction": clampf(float(rules.get("healthPercent", 100.0)) / 100.0, 0.0, 1.0),
		"anchor_filter": sim._typed_contract_tokens(fields, "AutoRespawnAtObjectFilter"),
		"respawn_as_template": String(sim._module_contract_value(fields, "RespawnAsTemplate", "")),
		"entries": (fields.get("RespawnEntry", []) as Array).duplicate(true),
		"death_animation_ticks": sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "DeathAnimationTime", 0.0))),
		"respawn_animation_ticks": sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "RespawnAnimationTime", 0.0))),
...
func _schedule_respawn_update(entity_id: int, row: Dictionary, death_type:String="NORMAL", attacker_id:int=0) -> void:
	...
	for entry_value in policy.get("entries", []) as Array:
		...
		if int(entry.get("level", 0)) == level:
			...
			delay = sim._ship_contract_delay_ticks(float(entry.get("timeMilliseconds", delay * sim.TICK_SECONDS * 1000.0)))
			break
...
func _attach_fire_weapon_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for nugget_value in fields.get("FireWeaponNugget", []) as Array:
		...
		var delay = sim._ship_contract_delay_ticks(float(sim._module_contract_value(nugget, "FireDelay", 0.0)))
		var offset := Vector2.ZERO
		var offset_z := 0.0
		var offset_value: Variant = sim._module_contract_value(nugget, "Offset", null)
		...
		nuggets.append({"weapon":String(sim._module_contract_value(nugget,"WeaponName","")),"one_shot":bool(sim._module_contract_value(nugget,"OneShot",false)),"delay_ticks":maxi(1,delay),"next_fire_tick":sim.tick_index+delay,"offset_source":offset,"offset_z_source":offset_z,"fired":false})
	row["fire_weapon_updates"] = {
		"charging_trigger": bool(sim._module_contract_value(fields,"ChargingModeTrigger",false)),
		"alive_only": bool(sim._module_contract_value(fields,"AliveOnly",false)),
		"hero_mode_trigger": bool(sim._module_contract_value(fields,"HeroModeTrigger",false)),
		"nuggets": nuggets,
	}
...
func _attach_deletion_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if not bool(low.get("resolved",false)) or not bool(high.get("resolved",false)):
		...
		return
	var low_ticks=sim._ship_contract_delay_ticks(float(low.get("milliseconds",0.0)))
	var high_ticks=maxi(low_ticks,sim._ship_contract_delay_ticks(float(high.get("milliseconds",0.0))))
...
func _step_deletion_updates() -> void:
	for id in sim.entity_ids():
		...
		if not row.has("deletion_update") and not row.has("module_contracts"):sim._attach_module_contracts(row)
		...
		if not bool(policy.get("indefinite",false)) and int(policy.get("expire_tick",-1))>=0 and sim.tick_index>=int(policy.get("expire_tick",-1)):
			sim._expire_lifetime_entity(id,row,"FADED")
	for id in sim.structure_ids():
		var row:=sim.structures[id] as Dictionary
		if not row.has("deletion_update") and not row.has("module_contracts"):sim._attach_structure_module_contracts(row)
		var policy:=row.get("deletion_update",{}) as Dictionary
		if not bool(policy.get("indefinite",false)) and int(policy.get("expire_tick",-1))>=0 and sim.tick_index>=int(policy.get("expire_tick",-1)):
			sim._expire_lifetime_structure(id,row,"FADED")
...
func _attach_production_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for value in fields.get("ProductionModifier",[]) as Array:
		...
		modifiers.append({"required_upgrade":String(sim._module_contract_value(source,"RequiredUpgrade","")),"cost_multiplier":float(sim._module_contract_value(source,"CostMultiplier",1.0)),"time_multiplier":float(sim._module_contract_value(source,"TimeMultiplier",1.0)),"filter":sim._typed_contract_tokens(source,"ModifierFilter"),"hero_purchase":bool(sim._module_contract_value(source,"HeroPurchase",false)),"hero_revive":bool(sim._module_contract_value(source,"HeroRevive",false))})
	var receipts:Array[String]=[]
	...
	row["production_update"]={"maximum_queue_entries":int(sim._module_contract_value(fields,"MaxQueueEntries",0)),"give_no_xp":bool(sim._module_contract_value(fields,"GiveNoXP",false)),"veteran_units":bool(sim._module_contract_value(fields,"VeteranUnitsFromVeteranFactory",false)),"unit_invulnerable_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"UnitInvulnerableTime",0.0))),"modifiers":modifiers,"unsupported_semantics":receipts}
...
func _attach_getting_built_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	row["getting_built_behavior"]={"spawn_disabled":bool(spawn.get("disabled",false)),"spawn_ticks":int(spawn.get("ticks",-1)),"rebuild_ticks":int(rebuild.get("ticks",-1)),"rebuild_when_dead":bool(sim._module_contract_value(fields,"RebuildWhenDead",false)),"use_timer_without_worker":bool(sim._module_contract_value(fields,"UseSpawnTimerWithoutWorker",false)),"disallow_range_source":float(sim._module_contract_value(fields,"DisallowRebuildRange",0.0)),"disallow_filter":sim._typed_contract_tokens(fields,"DisallowRebuildFilter"),"unsupported_semantics":receipts}
...
func _resolve_build_seconds(value:Variant)->Dictionary:
	...
	if field.has("milliseconds"):return {"resolved":true,"disabled":false,"ticks":sim._ship_contract_delay_ticks(float(field.get("milliseconds",0.0)))}
	var name:=String(field.get("define",""));var defines:=sim._rules.get("getting_built_time_defines",{}) as Dictionary
	if typeof(defines.get(name)) in [TYPE_INT,TYPE_FLOAT]:return {"resolved":true,"disabled":false,"ticks":sim._ship_contract_delay_ticks(float(defines[name])*1000.0)}
...
func _attach_building_behavior_contract(row:Dictionary,contract:Dictionary)->void:
	...
	row["building_behavior"]={
		"night_windows":sim._typed_contract_tokens(fields,"NightWindowName"),
		"fire_windows":sim._typed_contract_tokens(fields,"FireWindowName"),
		"glow_windows":sim._typed_contract_tokens(fields,"GlowWindowName"),
...
func _attach_queue_production_exit_contract(row:Dictionary,contract:Dictionary)->void:
	...
	for value in fields.get("ExitDelay",[]) as Array:
		...
		if typeof(delay.get("milliseconds")) in [TYPE_INT,TYPE_FLOAT]:delays.append(sim._ship_contract_delay_ticks(float(delay.get("milliseconds",0.0))))
		else:
			var expression:=String(delay.get("expression",""));var defines:=sim._rules.get("queue_exit_time_defines",{}) as Dictionary
			if typeof(defines.get(expression)) in [TYPE_INT,TYPE_FLOAT]:delays.append(sim._ship_contract_delay_ticks(float(defines[expression])))
...
func _attach_rebuild_hole_expose_contract(row: Dictionary, contract: Dictionary) -> void:
	if String(contract.get("extraction", "")) != "typed" or row.has("rebuild_hole_expose"):
		return
	var fields := contract.get("fields", {}) as Dictionary
	var hole_name = String(sim._module_contract_value(fields, "HoleName", ""))
	var hole_health = int(round(float(sim._module_contract_value(fields, "HoleMaxHealth", 0.0))))
	if hole_name == "" or hole_health <= 0:
		return
	var transfer_attackers = bool(sim._module_contract_value(fields, "TransferAttackers", false))
	var fade_in_ticks = maxi(0, sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "FadeInTimeSeconds", 0.0)) * 1000.0))
	var receipts: Array[String] = []
	if fade_in_ticks > 0:
	...
	row["rebuild_hole_expose"] = {
		...
		"exempt_statuses": sim._typed_contract_tokens(fields, "ExemptStatus"),
...
func _attach_rebuild_hole_behavior_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	row["rebuild_hole_behavior"] = {
		"worker_object_id": String(sim._module_contract_value(fields, "WorkerObjectName", "")),
		"respawn_ticks": sim._ship_contract_delay_ticks(float(delay_value.get("milliseconds", 0.0))),
...
func _expose_rebuild_hole(owner_id: int, owner: Dictionary, attacker_id: int) -> int:
	...
	for status in policy.get("exempt_statuses", []) as Array:
		if statuses.has(String(status).to_upper()):
			return -1
	var hole_id = sim.spawn_scenario_structure(
...
func _step_rebuild_holes() -> void:
	for hole_id in sim.structure_ids():
		...
		if sim.tick_index < int(policy.get("respawn_tick", sim.tick_index + 1)):
			...
			continue
		var rebuilt_id = sim.spawn_scenario_structure(
...
func _attach_banner_carrier_update_contract(row:Dictionary,contract:Dictionary)->void:
	...
	row["banner_carrier_update"]={"idle_spawn_ticks":maxi(1,sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"IdleSpawnRate",0.0)))),"melee_unit_spawn_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"MeleeFreeUnitSpawnTime",0.0))),"has_respawn_timer":fields.has("DiedRespawnTime") or fields.has("MeleeFreeBannerRespawnTime"),"died_respawn_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"DiedRespawnTime",0.0))),"melee_banner_respawn_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"MeleeFreeBannerRespawnTime",0.0))),"upgrade_required":String(sim._module_contract_value(fields,"UpgradeRequired","")),"replenish_nearby":bool(sim._module_contract_value(fields,"ReplenishNearbyHorde",false)),"replenish_all":bool(sim._module_contract_value(fields,"ReplenishAllNearbyHordes",false)),"scan_range_source":float(scan.get("value",0.0)) if bool(scan.get("resolved",false)) else -1.0,"next_replenish_tick":sim.tick_index,"unsupported_semantics":receipts}
...
func _attach_respawn_body_contract(row:Dictionary,contract:Dictionary)->void:
	...
	row["respawn_body"]={"max_health":int(maximum.get("value",0.0)) if bool(maximum.get("resolved",false)) else -1,"recovery_ticks":sim._ship_contract_delay_ticks(float(recovery.get("value",0.0))) if bool(recovery.get("resolved",false)) else -1,"max_health_damaged":int(damaged.get("value",0.0)) if bool(damaged.get("resolved",false)) else -1,"can_respawn":bool(sim._module_contract_value(fields,"CanRespawn",true)),"permanently_killed_filter":sim._typed_contract_tokens(fields,"PermanentlyKilledByFilter"),"cheer_radii":_typed_contract_numbers(fields,"CheerRadius"),"unsupported_semantics":receipts}
...
func _resolve_contract_milliseconds(field:Variant,define_key:String)->Dictionary:
	if typeof(field)!=TYPE_DICTIONARY:return {"resolved":false}
	var value:=field as Dictionary
	if typeof(value.get("milliseconds")) in [TYPE_INT,TYPE_FLOAT]:return {"resolved":true,"ticks":sim._ship_contract_delay_ticks(float(value.get("milliseconds")))}
	var expression:=String(value.get("expression",value.get("define","")));var defines:=sim._rules.get(define_key,{}) as Dictionary
	if typeof(defines.get(expression)) in [TYPE_INT,TYPE_FLOAT]:return {"resolved":true,"ticks":sim._ship_contract_delay_ticks(float(defines[expression]))}
...
func _attach_give_upgrade_contract(row:Dictionary,contract:Dictionary)->void:
	...
	for key in ["SpawnOutFX","FadeOutSpeed"]:
		if fields.has(key):receipts.append("presentation_binding:%s"%key)
	if bool(sim._module_contract_value(fields,"ApproachRequiresLOS",false)):receipts.append("unsupported_targeting_semantic:ApproachRequiresLOS")
	var rows:=row.get("give_upgrade_updates",[]) as Array
	rows.append({"special_power":String(sim._module_contract_value(fields,"SpecialPowerTemplate","")),"range_source":float(sim._module_contract_value(fields,"StartAbilityRange",0.0)),"deliver_upgrade":bool(sim._module_contract_value(fields,"DeliverUpgrade",false)),"phase_ticks":phases,"unsupported_semantics":receipts,"tag":String(contract.get("tag","")),"line":int(contract.get("line",0))});row["give_upgrade_updates"]=rows
...
func request_give_upgrade(source_id:int,target_id:int,upgrade_id:String,special_power:String="")->Dictionary:
	...
	if not source.has("give_upgrade_updates"):sim._attach_module_contracts(source)
...
func _attach_gate_open_close_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var opened=bool(sim._module_contract_value(fields,"OpenByDefault",false));row["gate_behavior"]={"open":opened,"pathing_open":opened,"open_fraction":1.0 if opened else 0.0,"reset_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"ResetTimeInMilliseconds",0.0))),"pathing_threshold":float(sim._module_contract_value(fields,"PercentOpenForPathing",100.0))/100.0,"repel_colliding":bool(sim._module_contract_value(fields,"RepelCollidingUnits",false)),"close_tick":-1,"unsupported_semantics":receipts}
...
func _attach_ai_gate_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var fields=contract.get("fields",{}) as Dictionary;row["ai_gate_update"]={"trigger_width_source":Vector2(float(sim._module_contract_value(fields,"TriggerWidthX",0.0)),float(sim._module_contract_value(fields,"TriggerWidthY",0.0)))}
...
func _attach_fake_pathfind_portal_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var fields=contract.get("fields",{}) as Dictionary;row["fake_pathfind_portal"]={"allow_enemies":bool(sim._module_contract_value(fields,"AllowEnemies",false)),"allow_non_skirmish_ai":bool(sim._module_contract_value(fields,"AllowNonSkirmishAIUnits",false))}
...
func request_gate_open(structure_id:int,requester_id:int=0)->Dictionary:
	...
	if not gate.has("gate_behavior"):sim._attach_structure_module_contracts(gate)
...
func _attach_stop_special_power_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields := contract.get("fields", {}) as Dictionary
	var own_template = String(sim._module_contract_value(fields, "SpecialPowerTemplate", "")).strip_edges()
	var stop_template = String(sim._module_contract_value(fields, "StopPowerTemplate", "")).strip_edges()
	if (
...
func _attach_unleash_special_power_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var unpack_ms = int(sim._module_contract_value(fields, "UnpackTime", -1))
	var award_xp = float(sim._module_contract_value(fields, "AwardXPForTriggering", -1.0))
	var instant_value: Variant = sim._module_contract_value(fields, "Instant", null)
	...
	policies.append({
		...
		"unpack_ticks": sim._ship_contract_delay_ticks(float(unpack_ms)),
		"award_xp": award_xp,
...
func activate_unleash_special_power(owner_id: int, special_power_template: String, team: int = -1) -> Dictionary:
	...
	if not owner.has("unleash_special_powers"):
		if table == sim.structures: sim._attach_structure_module_contracts(owner)
		else: sim._attach_module_contracts(owner)
	var policy: Dictionary = {}
...
func _attach_special_enemy_sense_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var fields := contract.get("fields", {}) as Dictionary
	var filter_value: Variant = sim._module_contract_value(fields, "SpecialEnemyFilter", null)
	var range_value: Variant = sim._module_contract_value(fields, "ScanRange", null)
	var interval_ms_value: Variant = sim._module_contract_value(fields, "ScanInterval", null)
...
func _attach_invisibility_update_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var invisibility_type = String(sim._module_contract_value(nugget, "InvisibilityType", "")).to_upper()
	var update_ms = float(sim._module_contract_value(fields, "UpdatePeriod", 0.0))
	...
	var broadcast = bool(sim._module_contract_value(fields, "Broadcast", false))
	var broadcast_filter = sim._typed_contract_raw_tokens(fields, "BroadcastObjectFilter")
	...
	var forbidden = sim._typed_contract_tokens(nugget, "ForbiddenConditions")
	if forbidden.has("AWAY_FROM_TREES"):
		receipts.append("environment-condition-unresolved:AWAY_FROM_TREES")
	if not sim._typed_contract_tokens(nugget, "HintDetectableConditions").is_empty():
	...
	row["invisibility_update"] = {
		"enabled": bool(sim._module_contract_value(fields, "StartsActive", false)),
		"starts_active": bool(sim._module_contract_value(fields, "StartsActive", false)),
		"update_ticks": maxi(1, sim._ship_contract_delay_ticks(update_ms)),
		"next_update_tick": sim.tick_index,
		"required_upgrades": sim._typed_contract_raw_tokens(fields, "RequiredUpgrades"),
		"forbidden_upgrades": sim._typed_contract_raw_tokens(fields, "ForbiddenUpgrades"),
		"broadcast": broadcast,
		...
		"forbidden_weapon_conditions": sim._typed_contract_tokens(nugget, "ForbiddenWeaponConditions"),
		"hint_detectable_conditions": sim._typed_contract_tokens(nugget, "HintDetectableConditions"),
		"options": sim._typed_contract_tokens(nugget, "Options"),
		...
		"become_fx_id": String(sim._module_contract_value(nugget, "BecomeStealthedFX", "")),
		"exit_fx_id": String(sim._module_contract_value(nugget, "ExitStealthFX", "")),
		"voice_move_role": String(sim._module_contract_value(fields, "UnitSpecificSoundNameToUseAsVoiceMoveToStealthyArea", "")),
		"voice_enter_role": String(sim._module_contract_value(fields, "UnitSpecificSoundNameToUseAsVoiceEnterStateMoveToStealthyArea", "")),
		"granted_ids": [],
		"unsupported_semantics": receipts,
		"tag": String(contract.get("tag", "")),
		"source_ini": String(contract.get("sourceIni", contract.get("source_ini", ""))),
...
func set_invisibility_update_active(object_id: int, enabled: bool, tag: String = "") -> Dictionary:
	...
	if not row.has("invisibility_update"):
		if sim.structures.has(object_id): sim._attach_structure_module_contracts(row)
		else: sim._attach_module_contracts(row)
...
func _set_invisibility_source(target: Dictionary, source_key: String, policy: Dictionary, enabled: bool, source_id: int) -> void:
	...
	if enabled and not was_hidden and sim.entities.has(int(target.get("id", 0))):
		...
		if voice_role != "": policy["last_audio_result"] = sim.emit_typed_audio_intent(int(target.get("id", 0)), voice_role)
...
func _attach_stealth_detector_contract(row:Dictionary,contract:Dictionary)->void:
	...
	if bool(sim._module_contract_value(fields,"CancelOneRingEffect",false)):receipts.append("unsupported_ring_presentation:CancelOneRingEffect")
	row["stealth_detector_update"]={"rate_ticks":int(rate.get("ticks",-1)),"range_source":float(range.get("value",0.0)) if bool(range.get("resolved",false)) else 0.0,"required_upgrade":String(sim._module_contract_value(fields,"RequiredUpgrade","")),"while_garrisoned":bool(sim._module_contract_value(fields,"CanDetectWhileGarrisoned",false)),"while_contained":bool(sim._module_contract_value(fields,"CanDetectWhileContained",false)),"next_tick":sim.tick_index,"unsupported_semantics":receipts}
...
func _attach_slaved_update_contract(row:Dictionary,contract:Dictionary)->void:
	...
	for key in ["UseSlaverAsControlForEvaObjectSightedEvents","FadeOutRange","FadeTime"]:
		if fields.has(key):receipts.append("presentation_binding:%s"%key)
	row["slaved_update"]={"master_id":0,"leash_range_source":float(sim._module_contract_value(fields,"LeashRange",0.0)),"guard_max_range_source":float(sim._module_contract_value(fields,"GuardMaxRange",0.0)),"guard_wander_range_source":float(sim._module_contract_value(fields,"GuardWanderRange",0.0)),"attack_range_source":float(sim._module_contract_value(fields,"AttackRange",0.0)),"guard_offset_source":offset,"die_on_master_death":bool(sim._module_contract_value(fields,"DieOnMastersDeath",false)),"mark_unselectable":bool(sim._module_contract_value(fields,"MarkUnselectable",false)),"unsupported_semantics":receipts}
...
func bind_slave(slave_id:int,master_id:int)->Dictionary:
	...
	var slave:=sim.entities[slave_id] as Dictionary
	if not slave.has("slaved_update"):sim._attach_module_contracts(slave)
...
func _attach_castle_upgrade_contract(row:Dictionary,contract:Dictionary)->void:
	if String(contract.get("extraction",""))!="typed":return
	var fields=contract.get("fields",{}) as Dictionary;var trigger=String(sim._module_contract_value(fields,"TriggeredBy","")).strip_edges();var upgrade=String(sim._module_contract_value(fields,"Upgrade","")).strip_edges()
...
func apply_castle_upgrade_trigger(structure_id:int,trigger_upgrade_id:String)->Dictionary:
	...
	if not building.has("castle_upgrade_contracts"):sim._attach_structure_module_contracts(building)
	var matched:=0
...
func _attach_spawn_behavior_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var can_reclaim=bool(sim._module_contract_value(fields,"CanReclaimOrphans",false))
	if can_reclaim:receipts.append("deferred_binary_ambiguous:CanReclaimOrphans")
	if bool(sim._module_contract_value(fields,"RespectCommandLimit",false)):receipts.append("unsupported_spawn_semantic:RespectCommandLimit")
	...
	row["spawn_behavior"]={"spawn_number":int(sim._module_contract_value(fields,"SpawnNumber",0)),"replace_ticks":sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"SpawnReplaceDelay",0.0))),"templates":templates,"one_shot":bool(sim._module_contract_value(fields,"OneShot",false)),"can_reclaim_orphans":can_reclaim,"can_reclaim_runtime":"bfme2-rotwk-binary-proven" if can_reclaim else "disabled","require_spawner":bool(sim._module_contract_value(fields,"SpawnedRequireSpawner",false)),"share_upgrades":bool(sim._module_contract_value(fields,"ShareUpgrades",false)),"triggered_by":String(sim._module_contract_value(fields,"TriggeredBy","")),"initial_remaining":int(sim._module_contract_value(fields,"InitialBurst",sim._module_contract_value(fields,"SpawnNumber",0))),"spawned_ids":[],"spawn_serial":0,"spawn_count":-1,"next_spawn_tick":sim.tick_index,"unsupported_semantics":receipts}
...
func _spawn_behavior_member(owner_id:int,owner:Dictionary,policy:Dictionary)->int:
	...
	if (unit_rules.get(template,{}) as Dictionary).is_empty():
		...
		spawned_id=sim.spawn_scenario_unit(template,team,Vector2(owner.get("position",Vector2.ZERO)),"lair-spawn")
...
func _attach_stealth_update_contract(row:Dictionary,contract:Dictionary)->void:
	...
	var enabled=bool(sim._module_contract_value(fields,"StartsActive",false)) or bool(sim._module_contract_value(fields,"InnateStealth",false));var delay=sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields,"StealthDelay",0.0)))
	row["stealth_update"]={"enabled":enabled,"delay_ticks":delay,"activation_tick":sim.tick_index+delay,"forbidden":sim._typed_contract_tokens(fields,"StealthForbiddenConditions"),"reveal_weapon_sets":sim._typed_contract_tokens(fields,"RevealWeaponSets"),"required_upgrades":required_upgrades,"detected_range_source":float(sim._module_contract_value(fields,"DetectedByAnyoneRange",0.0)),"reveal_target_range_source":float(sim._module_contract_value(fields,"RevealDistanceFromTarget",0.0)),"unsupported_semantics":receipts}
...
func set_stealth_update_active(entity_id:int,enabled:bool)->Dictionary:
	...
	var row=sim.entities[entity_id] as Dictionary;if not row.has("stealth_update"):sim._attach_module_contracts(row)
...
func _attach_object_creation_upgrade_contract(row:Dictionary,contract:Dictionary)->void:
	...
	for key in ["FadeInTime","DeathAnimAndDuration"]:
		if fields.has(key):receipts.append("presentation_binding:%s"%key)
	if bool(sim._module_contract_value(fields,"DestroyWhenSold",false)):receipts.append("unsupported_sale_semantic:DestroyWhenSold")
	if bool(sim._module_contract_value(fields,"UseBuildingProduction",false)):receipts.append("unsupported_creation_semantic:UseBuildingProduction")
	var offset:=Vector2.ZERO
	...
	rows.append({"triggers":triggers,"requires_all":bool(sim._module_contract_value(fields,"RequiresAllTriggers",false)),"conflicts":conflicts,"delay_ticks":int(delay.get("ticks",0)) if bool(delay.get("resolved",false)) else -1,"offset_source":offset,"thing_to_spawn":String(sim._module_contract_value(fields,"ThingToSpawn","")),"grant_upgrade":String(sim._module_contract_value(fields,"GrantUpgrade","")),"remove_upgrade":String(sim._module_contract_value(fields,"RemoveUpgrade","")),"upgrade_object":String(sim._module_contract_value(fields,"UpgradeObject","")),"scheduled_tick":-1,"consumed":false,"spawned_ids":[],"unsupported_semantics":receipts,"tag":String(contract.get("tag","")),"line":int(contract.get("line",0))});row["object_creation_upgrades"]=rows
...
func _attach_attribute_modifier_upgrade_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if String(contract.get("extraction", "")) != "typed":
		return
	var fields := contract.get("fields", {}) as Dictionary
	var triggers = sim._typed_contract_tokens(fields, "TriggeredBy")
	var modifier_name = String(sim._module_contract_value(fields, "AttributeModifier", "")).strip_edges()
	if triggers.is_empty() or modifier_name == "":
		return
	var requires_all_value: Variant = sim._module_contract_value(fields, "RequiresAllTriggers", false)
	...
	policies.append({
		...
		"conflicts": sim._typed_contract_tokens(fields, "ConflictsWith"),
...
func _attribute_modifier_upgrade_owned(row: Dictionary, upgrade_id: String) -> bool:
	if sim._aura_has_upgrade(row.get("completed_upgrades", []) as Array, row.get("applied_upgrades", {}) as Dictionary, upgrade_id):
		return true
	return sim._aura_has_upgrade([], sim.team_upgrades.get(int(row.get("team", -1)), {}) as Dictionary, upgrade_id)
...
func _attach_geometry_upgrade_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	policies.append({
		...
		"requires_all": bool(requires_all_value),
		"conflicts": sim._typed_contract_tokens(fields, "ConflictsWith"),
		"show_geometry": sim._typed_contract_raw_tokens(fields, "ShowGeometry"),
		"hide_geometry": sim._typed_contract_raw_tokens(fields, "HideGeometry"),
		"active": false,
		"unsupported_semantics": unsupported,
		"tag": tag,
		"line": line,
...
func _attach_emotion_tracker_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	for emotion_row_value in emotion_value as Array:
		...
		if emotion_row.has("Duration"):
			...
			if typeof(duration.get("milliseconds")) not in [TYPE_INT, TYPE_FLOAT]:
				return
			duration_ticks = sim._ship_contract_delay_ticks(float(duration.get("milliseconds")))
	...
	row["emotion_tracker"] = {
		"afraid_of": sim._typed_contract_tokens(fields, "AfraidOf"),
		"always_afraid_of": sim._typed_contract_tokens(fields, "AlwaysAfraidOf"),
		...
		"taunt_update_ticks": sim._ship_contract_delay_ticks(float(sim._module_contract_value(fields, "TauntAndPointUpdateDelay", 0.0))),
		"quarrel_fraction": quarrel_fraction,
		"immune_to_fear_level": int(sim._module_contract_value(fields, "ImmuneToFearLevel", 0)),
		"ignore_veterancy": bool(sim._module_contract_value(fields, "IgnoreVeterancy", false)),
		"emotions": emotions,
		"unsupported_semantics": unsupported,
