...
func _team_defeated(team: int) -> bool:
	...
	if _sim.base_loop_enabled:
		...
		return fortress != 0 and int((_sim.structures[fortress] as Dictionary).get("health", 0)) <= 0
	return _sim.living_ids(team).is_empty()
...
func _evaluate_cah_match_awards() -> void:
	var _sim = sim
	var unit_types: Array = _sim._cah_award_contracts.keys()
	...
	for unit_type_value in unit_types:
		...
		if _sim.cah_award_results.has(unit_type):
			continue
		var contract = _sim._cah_award_contracts[unit_type] as Dictionary
		var tally = _sim._cah_tally_for(unit_type)
		var owner_team = int(contract.get("ownerTeam", _sim._created_hero_owner_teams.get(unit_type, -1)))
		var mode_suffix = "OPENPLAY_MP" if _sim._cah_openplay_multiplayer() else "SKIRMISH"
		var result_stat = "HERO_%s_COUNT_%s" % ["VICTORY" if owner_team == _sim.winner else "DEFEAT", mode_suffix]
		...
		_sim.cah_award_results[unit_type] = result
		_sim._emit_event("cah.awards_resolved", 0, 0, result)
...
func _resolve_victory() -> void:
	...
	var _sim = sim
	for effect in _sim._pending_power_effects:
	...
	for physics_value in _sim.physics_objects.values():
		if String((physics_value as Dictionary).get("phase", "")) != "recovered":
			return
	for entity_value in _sim.entities.values():
		var lifetime := (entity_value as Dictionary).get("lifetime_update", {}) as Dictionary
		if int(lifetime.get("expire_tick", -1)) > _sim.tick_index:
			return
	for structure_value in _sim.structures.values():
	...
	for i in survivors.size():
		for j in range(i + 1, survivors.size()):
			if _sim._is_hostile(int(survivors[i]), int(survivors[j])):
				return
	if survivors.is_empty():
		var roster = _sim._roster_team_ids()
		_sim.winner = int(roster[0]) if not roster.is_empty() else _sim.PLAYER_TEAM
	else:
		_sim.winner = int(survivors[0])
	_evaluate_cah_match_awards()
	for id in _sim.living_ids(_sim.winner):
	...
	var observer_won = _sim.winner != -1 and not _sim._is_hostile(_sim.PLAYER_TEAM, _sim.winner)
	if observer_won:
		_sim._emit_event("match.victory", 0, 0)
		_sim._emit_event("eva.enemy_defeated", 0, 0, {"team": _sim.PLAYER_TEAM})
		_sim._emit_music("victory")
	else:
		_sim._emit_event("match.defeat", 0, 0)
		_sim._emit_event("eva.ally_defeated", 0, 0, {"team": _sim.PLAYER_TEAM})
		_sim._emit_music("defeat")
