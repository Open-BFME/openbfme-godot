...
func queue_unit(team: int, producer: int, unit_type: String = sim.SOLDIER_HORDE_ID) -> Dictionary:
	...
	if not sim.structures.has(producer):
		return {"ok": false, "reason": "unknown-producer"}
	var building: Dictionary = sim.structures[producer]
	...
	if production_rule.is_empty():
		return {"ok": false, "reason": "unsupported-unit"}
	if bool(production_rule.get("is_ring_hero", false)) and not sim.ring_mechanic_enabled:
		return {"ok": false, "reason": "ring-heroes-disabled"}
	if sim.created_hero_owner_team(unit_type) not in [-1, team]:
	...
	if String(production_rule.get("category", "")) == "hero" and sim.hero_unavailable(team, unit_type):
	...
	var missing_production_upgrade := production_gate_unsatisfied(
		...
		(sim.team_upgrades.get(team, {}) as Dictionary).keys(),
	)
	...
	var queued_command_points = sim._queued_command_points_for_team(team)
	if (
		sim.command_points_for_team(team) + queued_command_points + command_cost
		> sim.command_point_total_for_team(team)
	):
	...
	for modifier_value in production_contract.get("modifiers", []) as Array:
		...
		if not sim._structure_has_completed_upgrade(building, String(modifier.get("required_upgrade", ""))):
			continue
		var probe := {"category":String(production_rule.get("category", "")), "kind_of":production_rule.get("kind_of", [])}
		if not (modifier.get("filter", []) as Array).is_empty() and not sim._transport_filter_accepts(probe, modifier.get("filter", []) as Array):
			continue
	...
	var starts_at = sim.tick_index if queue.is_empty() else int((queue.back() as Dictionary).get("complete_tick", sim.tick_index))
	var item := {
		"unit_type": unit_type,
		"cost": cost,
	...
	sim._emit_event("production.queued", producer, 0, {"team": team, "unit_type": unit_type, "command_id": String(item.get("command_id", "")), "complete_tick": int(item["complete_tick"])})
...
func production_queue_state(producer: int) -> Array[Dictionary]:
	...
	if not sim.structures.has(producer):
		return rows
	var queue: Array = (sim.structures[producer] as Dictionary).get("queue", [])
	for index in range(queue.size()):
		if typeof(queue[index]) != TYPE_DICTIONARY:
			continue
		var item: Dictionary = queue[index]
		var start_tick := int(item.get("start_tick", item.get("queued_tick", sim.tick_index)))
		...
		var active := index == 0
		var elapsed_ticks := clampi(sim.tick_index - start_tick, 0, duration_ticks) if active else 0
		rows.append({
			"index": index,
			"unit_type": String(item.get("unit_type", sim.SOLDIER_HORDE_ID)),
...
func cancel_queued_unit(team: int, producer: int, queue_index: int = 0) -> Dictionary:
	...
	var cursor = sim.tick_index if queue_index == 0 else int((queue[queue_index - 1] as Dictionary).get("complete_tick", sim.tick_index))
	...
	sim._emit_event("production.cancelled", producer, 0, {
		"team": team,
		"unit_type": String(cancelled.get("unit_type", sim.SOLDIER_HORDE_ID)),
...
func _step_production() -> void:
	for id in sim.structure_ids():
		var building: Dictionary = sim.structures[id]
		...
		var new_id := int(sim._next_dynamic_id.get(team, 10 if team == sim.PLAYER_TEAM else 110))
		...
		if not authored_rallies.is_empty():rally=production_origin+sim._retail_source_to_sim_offset(Vector2(authored_rallies[exit_index%authored_rallies.size()]))
		var exit_direction := production_origin.direction_to(rally)
		if exit_direction.length_squared() <= 0.000001:
			exit_direction = Vector2.RIGHT if team == sim.PLAYER_TEAM else Vector2.LEFT
		var door_point = production_origin + exit_direction * sim.PRODUCTION_DOOR_INSET_RADIUS
		var create_point = production_origin + exit_direction * sim.PRODUCTION_EXIT_RADIUS
		if not authored_points.is_empty():create_point=production_origin+sim._retail_source_to_sim_offset(Vector2(authored_points[exit_index%authored_points.size()]))
		var unit_type := String(item.get("unit_type", sim.SOLDIER_HORDE_ID))
		...
		var production_rule: Dictionary = sim._unit_production_rules.get(unit_type, sim._unit_production_rules.get(sim.SOLDIER_HORDE_ID, {}))
		...
		if bool(production_rule.get("is_ring_hero", false)):
			var ring_hero: Dictionary = sim.entities[new_id]
			...
			for team_structure_id in sim.structure_ids(team):
				var team_structure: Dictionary = sim.structures[team_structure_id]
			...
			sim._emit_event("ring.hero_created", new_id, id, {"team": team, "rank": 10})
		if String(production_rule.get("category", "")) == "hero":
			sim._completed_hero_identities["%d:%s" % [team, unit_type]] = true
			if team == sim.PLAYER_TEAM:
				sim._emit_event("eva.hero_created", new_id, 0, {"team": team, "object_id": object_id, "unit_type": unit_type})
		var produced: Dictionary = sim.entities[new_id]
		produced["production_producer_id"] = id
		produced["production_exit_start_tick"] = sim.tick_index
		var authored_delays:=exit_contract.get("exit_delay_ticks",[]) as Array;var exit_ticks: int = sim.PRODUCTION_EXIT_DURATION_TICKS
		...
		if no_exit_path:
			...
			sim._spatial_sync(produced)
		if not exit_contract.is_empty():exit_contract["next_index"]=exit_index+1;building["queue_production_exit_update"]=exit_contract
		sim.team_command_points[team] = sim.command_points_for_team(team) + committed_command_points
		sim._emit_event("production.complete", id, new_id, {
			"team": team,
			"unit_type": unit_type,
			"object_id": object_id,
...
func _step_production_exit(row: Dictionary) -> bool:
	...
	if duration <= 0 or start_tick < 0:
		...
		return false
	var elapsed := maxi(0, sim.tick_index - start_tick)
	...
	if elapsed >= duration:
		...
		if not rally.is_equal_approx(exit_destination) and sim._assign_route(row, rally):
			row["state"] = "run"
		else:
			sim._clear_pending_route(row, true)
			row["state"] = "idle"
		sim._emit_event("production.exit_complete", int(row.get("production_producer_id", 0)), int(row.get("id", 0)), {
			"rally": rally,
...
func _release_command_points_once(row: Dictionary) -> void:
	if not sim.base_loop_enabled or bool(row.get("command_points_released", false)):
		return
	row["command_points_released"] = true
	var row_team := int(row.get("team", -1))
	sim.team_command_points[row_team] = maxi(
		0,
		sim.command_points_for_team(row_team)
...
func production_gate_unsatisfied(
	...
	for required_value in sim.required_upgrades_for_unit(unit_type, producer_kind):
	...
	var any_group = sim.required_upgrade_any_group_for_unit(unit_type, producer_kind)
...
func _production_rule_value(unit_type: String, rule_key: String, default_key: String) -> int:
	...
	return int(sim._rules.get(gameplay_rule, int(production_rule.get(default_key, 0))))
