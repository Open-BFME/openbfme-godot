...
func _step_entity(id: int) -> void:
	...
	if not (row.get("dominate_enemy_channel", {}) as Dictionary).is_empty():
		...
		_sim._rearm_mood_idle_cadence(row)
		return
	if _sim.tick_index < int(row.get("cower_until_tick", -1)):
		row["current_speed"] = 0.0
		row["state"] = "cower"
		_sim._rearm_mood_idle_cadence(row)
		return
		...
		row["state"] = "idle"
	if _sim.entity_container.has(id):
		...
		row["current_speed"] = 0.0
		var carrier_id = int(_sim.entity_container.get(id, 0))
		if _sim.structures.has(carrier_id):
			row["position"] = Vector2((_sim.structures[carrier_id] as Dictionary).get("position", row.get("position", Vector2.ZERO)))
		if not bool(row.get("contained_can_attack", false)) and not bool(horde_ai.get("can_attack_while_contained", false)):
			row["state"] = "contained"
			return
	if _sim.tick_index < int(row.get("stun_until_tick", -1)):
	...
	if knockdown_ticks > 0:
		...
		if not (row.get("capture_channel", {}) as Dictionary).is_empty():
			...
			_sim._emit_event("structure.capture_cancelled", id, int(channel.get("structure_id", 0)), {"team": int(row.get("team", -1))})
		knockdown_ticks -= 1
		row["knockdown_ticks"] = knockdown_ticks
		row["current_speed"] = 0.0
		if knockdown_ticks <= 0:
			row["knocked_down"] = false
			_sim._emit_event("combat.stand_up", id, 0)
			...
			if not _sim._resume_order_after_knockdown(row):
				row["state"] = "idle"
		else:
	...
	if not (row.get("siege_deploy_channel", {}) as Dictionary).is_empty():
		...
		_sim._rearm_mood_idle_cadence(row)
		return
	if _sim._step_capture_channel(row):
		_sim._rearm_mood_idle_cadence(row)
		return
	if _sim._step_volley_channel(row):
		_sim._rearm_mood_idle_cadence(row)
		return
	if _sim.tick_index < int(row.get("ability_hold_until_tick", -1)):
		...
		_sim._rearm_mood_idle_cadence(row)
		return
	if _sim._step_production_exit(row):
		_sim._rearm_mood_idle_cadence(row)
		return
	if bool(row.get("is_builder", false)):
		_sim._rearm_mood_idle_cadence(row)
		var construction_id := int(row.get("construction_id", 0))
		if construction_id != 0 and _sim.structures.has(construction_id):
			...
			if float(site.get("construction_progress", 1.0)) < 1.0:
				...
				var footprint = _sim._structure_placement_radius(String(site.get("structure_kind", "")))
	...
	if target_id == 0 and (row["route"] as Array).is_empty() and not bool(row.get("attack_move", false)):
		...
		if mood_rate > 0:
			var policy_allows_idle_scan = (
				...
				and (
					not _sim._stealth_active(row)
					or bool(row.get("auto_acquire_while_stealthed", true))
				)
			)
			scan_now = (
				policy_allows_idle_scan
				and _sim.tick_index >= int(row.get("mood_next_check_tick", -1))
			)
		if scan_now:
			var auto_target = _sim._nearest_auto_target(row)
			if mood_rate > 0:
				var interval := mood_rate
				if bool(row.get("mood_randomize_next_check", true)):
					var half_rate := mood_rate >> 1
					interval += _sim.logic_random_int(-half_rate, half_rate)
					row["mood_randomize_next_check"] = false
				row["mood_next_check_tick"] = _sim.tick_index + maxi(1, interval)
	...
	if target_id == 0 and bool(row.get("attack_move", false)) and not (row["route"] as Array).is_empty():
		var acquired = _sim._nearest_attack_move_target(row)
		if acquired != 0:
	...
	if target_id != 0:
		...
		if not _sim._target_alive(target_id, target_kind):
			...
			if bool(row.get("attack_move", false)) and _sim._assign_route(row, Vector2(row.get("attack_move_destination", row["position"]))):
				row["state"] = "run"
			else:
				_sim._clear_pending_route(row, true)
				row["state"] = "idle"
		...
		var auto_target_invalid = (
			...
			and (
				...
				or (
					_sim._stealth_active(row)
					and not bool(row.get("auto_acquire_while_stealthed", true))
				)
				or (
					target_kind == "battalion"
					and _sim._stealth_active(_sim.entities[target_id] as Dictionary)
				)
				or (
		...
		if auto_target_invalid:
			...
			row["state"] = "idle"
			return
		if target_kind == "battalion" and not _sim._can_engage_battalion(row, _sim.entities[target_id] as Dictionary):
			...
			row["state"] = "idle"
			return
		var target_position = _sim._target_position(target_id, target_kind)
		...
		var distance := maxf(
			...
			- _sim._target_footprint_radius(target_id, target_kind)
		)
		var selected_weapon_mode = _sim._weapon_mode_for_distance(row, distance)
		if selected_weapon_mode == "unsupported-close":
			...
			_sim._clear_member_attack_schedule(row)
			return
		_sim._apply_weapon_mode(row, selected_weapon_mode)
		...
		if distance <= float(row["attack_range"]) and (minimum_range <= 0.0 or distance >= minimum_range):
			row["state"] = "attack"
			_sim._clear_pending_route(row, true)
			_sim._step_member_attacks(id, row, target_id, target_kind)
			return
		row["attack_windup"] = 0
		_sim._clear_member_attack_schedule(row)
		if (
		...
		if minimum_range > 0.0 and distance < minimum_range:
			...
			_sim._clear_pending_route(row, true)
			if not _sim._assign_route(row, fallback):
				row["state"] = "idle"
				return
			row["state"] = "run"
			_sim._step_route(row)
		...
		if (row["route"] as Array).is_empty():
			if _sim.entity_container.has(id):
				row["state"] = "contained"
				return
			if not _sim._assign_target_route(row, target_position):
				row["target_id"] = 0
				_sim._clear_pending_route(row, true)
				row["state"] = "idle"
				return
...
func set_structure_rally(team: int, structure_id: int, position: Vector2) -> Dictionary:
	...
	row["rally"] = position
	_sim._emit_event("structure.rally_set", 0, structure_id, {"team": team})
	return {"ok": true}
...
func _step_battalion_separation() -> void:
	var _sim = sim
	var ids = _sim.entity_ids()
	for index in range(ids.size()):
		var a: Dictionary = _sim.entities[ids[index]]
		...
		var neighbours = _sim._spatial_gather_sorted(
			gather_origin, _sim.BATTALION_SEPARATION_RADIUS + _sim.BATTALION_SEPARATION_QUERY_SLACK
		)
		var widened := false
		...
		while true:
			...
			if not widened \
					and gather_origin.distance_to(Vector2(a["position"])) > _sim.BATTALION_SEPARATION_QUERY_SLACK:
				widened = true
				neighbours = ids
				cursor = 0
			...
			if not _sim.entities.has(other_id):
				continue
			var b: Dictionary = _sim.entities[other_id]
			...
			if distance >= _sim.BATTALION_SEPARATION_RADIUS or distance <= 0.001:
				continue
			...
			if int(a.get("target_id", 0)) == int(b.get("id", 0)) or int(b.get("target_id", 0)) == int(a.get("id", 0)):
				continue
			var push = offset / distance * minf(_sim.BATTALION_SEPARATION_PUSH, (_sim.BATTALION_SEPARATION_RADIUS - distance) * 0.5)
			...
			if _position_walkable(a_target):
				a["position"] = a_target
				_sim._spatial_sync(a)
			if _position_walkable(b_target):
				b["position"] = b_target
				_sim._spatial_sync(b)


func _position_walkable(position: Vector2) -> bool:
	var _sim = sim
	if _sim.route_provider == null or not _sim.route_provider.has_method("is_local_inside_navigation"):
		return true
	if not bool(_sim.route_provider.call("is_local_inside_navigation", position)):
		return false
	if _sim.route_provider.has_method("is_navigation_walkable") and _sim.route_provider.has_method("local_to_grid_cell"):
		return bool(_sim.route_provider.call("is_navigation_walkable", _sim.route_provider.call("local_to_grid_cell", position)))
	return true
...
func _step_hero_regeneration() -> void:
	...
	for id in _sim.entity_ids():
		...
		if health <= 0 or health >= maximum:
			continue
		var ticks_since_damage = _sim.tick_index - int(row.get("last_damage_tick", -1000000))
		if float(ticks_since_damage) * _sim.TICK_SECONDS < _sim.HERO_REGEN_OUT_OF_COMBAT_SECONDS:
			continue
		...
		var heal_rate = _sim.HERO_REGEN_PERCENT_PER_SECOND * float(row.get("auto_heal_multiplier", 1.0))
		var amount = maxi(1, roundi(float(maximum) * heal_rate * _sim.TICK_SECONDS))
