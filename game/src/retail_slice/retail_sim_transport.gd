...
func _attach_siege_engine_contain_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var slots: Variant = _sim._module_contract_value(fields, "Slots", null)
	...
	if bool(_sim._module_contract_value(fields, "ShowPips", false)):
	...
	if int(_sim._module_contract_value(fields, "NumberOfExitPaths", 0)) > 1:
	...
	row["horde_transport"] = {
		"module": "SiegeEngineContain", "contained_statuses": _sim._typed_contract_tokens(fields, "ObjectStatusOfContained"),
		"crew_statuses": _sim._typed_contract_tokens(fields, "ObjectStatusOfCrew"), "passenger_filter": _sim._typed_contract_tokens(fields, "PassengerFilter"),
		"crew_filter": _sim._typed_contract_tokens(fields, "CrewFilter"), "crew_max": int(_sim._module_contract_value(fields, "CrewMax", 0)),
		...
		"damage_ratio": float(damage.get("ratio", 0.0)), "allow_own": bool(_sim._module_contract_value(fields, "AllowAlliesInside", false)),
		"allow_allies": bool(_sim._module_contract_value(fields, "AllowAlliesInside", false)), "allow_enemies": bool(_sim._module_contract_value(fields, "AllowEnemiesInside", false)),
		"allow_neutral": bool(_sim._module_contract_value(fields, "AllowNeutralInside", false)), "exit_delay_ticks": _sim._ship_contract_delay_ticks(float(_sim._module_contract_value(fields, "ExitDelay", 0.0))),
		"kill_passengers_on_death": bool(_sim._module_contract_value(fields, "KillPassengersOnDeath", false)), "eject_passengers_on_death": bool(_sim._module_contract_value(fields, "EjectPassengersOnDeath", false)),
		"go_aggressive_on_exit": bool(_sim._module_contract_value(fields, "GoAggressiveOnExit", false)), "speed_fraction_per_crew": float((fields.get("SpeedPercentPerCrew", {}) as Dictionary).get("fraction", 0.0)),
		"weapon_sets_one": [String(_sim._module_contract_value(fields, "TypeOneForWeaponSet", ""))] if fields.has("TypeOneForWeaponSet") else [],
...
func _attach_container_family_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var capacity: Variant = _sim._module_contract_value(fields, capacity_key, null)
	...
	var kill_on_death := bool(_sim._module_contract_value(fields, "KillPassengersOnDeath", false))
	row["transport_capacity"] = int(capacity)
	row["horde_transport"] = {
		"module": module,
		"contained_statuses": _sim._typed_contract_tokens(fields, "ObjectStatusOfContained"),
		"passenger_filter": _sim._typed_contract_tokens(fields, "PassengerFilter"),
		"manual_pickup_filter": _sim._typed_contract_tokens(fields, "ManualPickUpFilter"),
		...
		"exit_delay_ticks": _sim._ship_contract_delay_ticks(float(_sim._module_contract_value(fields, "ExitDelay", 0.0))),
		"kill_passengers_on_death": kill_on_death,
		"eject_passengers_on_death": bool(_sim._module_contract_value(fields, "EjectPassengersOnDeath", false)) or (module in ["GarrisonContain", "HordeGarrisonContain"] and not kill_on_death),
		"force_orientation": bool(_sim._module_contract_value(fields, "ForceOrientationContainer", false)),
...
func _attach_horde_transport_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if bool(_sim._module_contract_value(fields, "ShowPips", false)):
		unsupported.append("show_pips_requires_hud_binding")
	if bool(_sim._module_contract_value(fields, "FadePassengerOnEnter", false)) or bool(_sim._module_contract_value(fields, "FadePassengerOnExit", false)):
	...
	row["horde_transport"] = {
		"contained_statuses": _sim._typed_contract_tokens(fields, "ObjectStatusOfContained"),
		"passenger_filter": _sim._typed_contract_tokens(fields, "PassengerFilter"),
		"fade_filter": _sim._typed_contract_tokens(fields, "FadeFilter"),
		...
		"exit_delay_ticks": _sim._ship_contract_delay_ticks(float(_sim._module_contract_value(fields, "ExitDelay", 0.0))),
		"force_orientation": bool(_sim._module_contract_value(fields, "ForceOrientationContainer", false)),
		"show_pips": bool(_sim._module_contract_value(fields, "ShowPips", false)),
		"kill_passengers_on_death": bool(_sim._module_contract_value(fields, "KillPassengersOnDeath", false)),
		"eject_passengers_on_death": bool(_sim._module_contract_value(fields, "EjectPassengersOnDeath", false)),
		"fade_on_enter": bool(_sim._module_contract_value(fields, "FadePassengerOnEnter", false)),
		"enter_fade_ticks": _sim._ship_contract_delay_ticks(float(_sim._module_contract_value(fields, "EnterFadeTime", 0.0))),
		"fade_on_exit": bool(_sim._module_contract_value(fields, "FadePassengerOnExit", false)),
		"exit_fade_ticks": _sim._ship_contract_delay_ticks(float(_sim._module_contract_value(fields, "ExitFadeTime", 0.0))),
		"passenger_bones": (bones_value as Array).duplicate(true),
		"enter_sound": String(_sim._module_contract_value(fields, "EnterSound", "")),
		"exit_sound": String(_sim._module_contract_value(fields, "ExitSound", "")),
		"unsupported_semantics": unsupported,
		"tag": String(contract.get("tag", "")),
		"line": int(contract.get("line", 0)),
	}
...
func _attach_ship_slow_death_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if String(contract.get("extraction", "")) != "typed" or row.has("ship_slow_death"):
		return
	var fields: Dictionary = contract.get("fields", {}) as Dictionary
	var sink_delay: Variant = _sim._module_contract_value(fields, "SinkDelay", null)
	var destruction: Variant = _sim._module_contract_value(fields, "DestructionDelay", null)
	var sink_rate: Variant = _sim._module_contract_value(fields, "SinkRate", null)
	...
	row["ship_slow_death"] = {
		...
		"sink_delay_ticks": _sim._ship_contract_delay_ticks(float(sink_delay)),
		"destruction_delay_ticks": _sim._ship_contract_delay_ticks(float(destruction)),
...
func _attach_slow_death_core_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	if mode not in ["ALL", "NONE"]:
		return
	var probability := int(_sim._module_contract_value(fields, "ProbabilityModifier", 10))
	var sink_delay_ms := float(_sim._module_contract_value(fields, "SinkDelay", 0.0))
	var sink_rate := float(_sim._module_contract_value(fields, "SinkRate", 0.0))
	var destruction_delay_ms := float(
		_sim._module_contract_value(fields, "DestructionDelay", 0.0)
	)
...
func load_transport_entity(carrier_id: int, entity_id: int, manual_pickup: bool = false) -> Dictionary:
	var _sim = sim
	if not _sim.structures.has(carrier_id) or not _sim.entities.has(entity_id):
		return {"ok": false, "reason": "carrier-or-passenger-missing"}
	var carrier := _sim.structures[carrier_id] as Dictionary
	if not carrier.has("horde_transport"):
		_sim._attach_structure_module_contracts(carrier)
	if not carrier.has("horde_transport"):
	...
	var occupied_slots = _sim.passenger_count(carrier_id)
	...
	var relation = _sim.team_relationship(int(carrier.get("team", -1)), int(passenger.get("team", -2)))
	...
	var result = _sim.contain_entity(carrier_id, entity_id)
	...
	passenger["position"] = Vector2(carrier.get("position", Vector2.ZERO)) if garrison_module else Vector2(carrier.get("position", Vector2.ZERO)) + _sim._retail_source_to_sim_offset(Vector2(policy.get("entry_offset_source", Vector2.ZERO)))
	...
	if garrison_module:
		...
		if tower_vision_source > 0.0:
			passenger["vision_range"] = tower_vision_source * _sim.source_transform_scale()
	if int(carrier.get("team", -1)) == _sim.CASTLE_CIVILIAN_TEAM and bool(policy.get("allow_neutral", false)):
		carrier["team"] = int(passenger.get("team", -1))
		_sim._emit_event("garrison.neutral_captured", entity_id, carrier_id, {"team": int(carrier.get("team", -1))})
	...
	if bool(policy.get("fade_on_enter", false)) and _transport_filter_accepts(passenger, policy.get("fade_filter", []) as Array):
		passenger["transport_fade_until_tick"] = _sim.tick_index + int(policy.get("enter_fade_ticks", 0))
	var carrier_object_id := String(carrier.get("source_object_id", ""))
	var passenger_object_id := String(passenger.get("object_id", ""))
	_sim._emit_event("transport.enter", carrier_id, entity_id, {
...
func issue_garrison(team: int, entity_id: int, structure_id: int) -> Dictionary:
	var _sim = sim
	if not _sim.entities.has(entity_id) or int((_sim.entities[entity_id] as Dictionary).get("team", -1)) != team:
...
func issue_exit_garrison(team: int, entity_id: int) -> Dictionary:
	...
	var carrier_id := int(_sim.entity_container[entity_id])
...
func _transport_entry_voice_candidates(passenger_object_id: String, carrier_object_id: String) -> Array[String]:
	...
	if passenger_object_id == "" or carrier_object_id == "":
		return []
	var runtimes_value: Variant = _sim._rules.get("playable_unit_runtimes", {})
	...
	for key in keys:
		...
		if (
			...
			and _sim.PlayableUnitAdapter.runtime_unit_id(document).to_lower() != folded
			and _sim.PlayableUnitAdapter.runtime_member_id(document).to_lower() != folded
		):
			continue
		return _sim.PlayableUnitAdapter.transport_entry_audio_event_ids(document, carrier_object_id)
	return []
...
func load_siege_crew(carrier_id: int, entity_id: int) -> Dictionary:
	...
	var crew := _sim.entities[entity_id] as Dictionary
...
func _update_siege_crew_state(carrier_id: int) -> void:
	var _sim = sim
	if not _sim.structures.has(carrier_id):
		return
	var carrier := _sim.structures[carrier_id] as Dictionary
	var policy := carrier.get("horde_transport", {}) as Dictionary
	var live: Array = []
	for value in carrier.get("siege_crew_ids", []) as Array:
		...
		if _sim.entity_container.get(crew_id, -1) == carrier_id and _sim.entities.has(crew_id) and int((_sim.entities[crew_id] as Dictionary).get("health", 0)) > 0:
			live.append(crew_id)
...
func request_transport_exit(entity_id: int) -> Dictionary:
	...
	pending[entity_id] = _sim.tick_index + int(policy.get("exit_delay_ticks", 0))
...
func request_tunnel_exit(entity_id: int, destination_id: int) -> Dictionary:
	...
	if not _sim.entity_container.has(entity_id):
		return {"ok": false, "reason": "not-contained"}
	var source_id := int(_sim.entity_container[entity_id])
	if not _sim.structures.has(source_id) or not _sim.structures.has(destination_id):
		return {"ok": false, "reason": "tunnel-missing"}
	var source := _sim.structures[source_id] as Dictionary
	var destination := _sim.structures[destination_id] as Dictionary
	...
	var exit_tick = _sim.tick_index + int(source_policy.get("exit_delay_ticks", 0))
...
func _step_ship_runtime() -> void:
	var _sim = sim
	var carrier_ids = _sim.structures.keys()
	carrier_ids.sort()
	for carrier_id_value in carrier_ids:
		...
		if String(carrier.get("ship_death_phase", "")) != "sinking":
			continue
		if _sim.tick_index >= int(carrier.get("ship_sink_start_tick", 0)):
			carrier["height_source"] = float(carrier.get("height_source", 0.0)) - float(carrier.get("ship_sink_rate_source_per_second", 0.0)) * _sim.TICK_SECONDS
		if _sim.tick_index >= int(carrier.get("ship_destroy_tick", 0)):
			for passenger_value in (_sim.containment.get(carrier_id, []) as Array).duplicate():
				_finish_transport_exit(carrier_id, int(passenger_value))
			_sim.structures.erase(carrier_id)
			_sim._emit_event("ship.destroyed", carrier_id, 0)
...
func _finish_transport_exit(carrier_id: int, entity_id: int, destination_id: int = -1) -> void:
	...
	if not _sim.entities.has(entity_id):
		...
		return
	var passenger := _sim.entities[entity_id] as Dictionary
	var carrier := _sim.structures.get(carrier_id, {}) as Dictionary
	var policy := carrier.get("horde_transport", {}) as Dictionary
	...
	var destination := _sim.structures.get(destination_id, carrier) as Dictionary
	var destination_policy := destination.get("horde_transport", policy) as Dictionary
	passenger["position"] = Vector2(destination.get("position", passenger.get("position", Vector2.ZERO))) + _sim._retail_source_to_sim_offset(Vector2(destination_policy.get("exit_offset_source", Vector2.ZERO)))
	if bool(policy.get("fade_on_exit", false)) and _transport_filter_accepts(passenger, policy.get("fade_filter", []) as Array):
		passenger["transport_fade_until_tick"] = _sim.tick_index + int(policy.get("exit_fade_ticks", 0))
	if was_siege_crew:
		var crew_ids := carrier.get("siege_crew_ids", []) as Array
	...
	_sim._emit_event("transport.exit", carrier_id, entity_id, {"sound": String(policy.get("exit_sound", ""))})
	_update_container_weapon_state(carrier_id)
...
func _update_container_weapon_state(carrier_id: int) -> void:
	...
	var carrier := _sim.structures[carrier_id] as Dictionary
	var policy := carrier.get("horde_transport", {}) as Dictionary
	var count = _sim.passenger_count(carrier_id)
...
func _apply_transport_passenger_damage(attacker_id: int, passenger_id: int, amount: int) -> void:
	...
	var _sim = sim
	if amount <= 0 or not _sim.entities.has(passenger_id):
		return
	...
	if total <= 0:
		...
		_sim._apply_playable_unit_death_policy(passenger, "NORMAL", no_defeated_members)
		_sim._emit_event("transport.passenger_killed", attacker_id, passenger_id)
...
func _begin_ship_slow_death(carrier_id: int, carrier: Dictionary, death_type: String) -> bool:
	...
	if policy.is_empty() or not _sim._death_mux_matches(policy, death_type):
		return false
	carrier["ship_death_phase"] = "sinking"
	carrier["ship_sink_start_tick"] = _sim.tick_index + int(policy.get("sink_delay_ticks", 0))
	carrier["ship_destroy_tick"] = _sim.tick_index + int(policy.get("destruction_delay_ticks", 0))
	...
	_sim._emit_event("ship.sinking", carrier_id, 0, {"sound": sound_event})
	var transport := carrier.get("horde_transport", {}) as Dictionary
	for passenger_value in (_sim.containment.get(carrier_id, []) as Array).duplicate():
		var passenger_id := int(passenger_value)
		if bool(transport.get("kill_passengers_on_death", false)) and _sim.entities.has(passenger_id):
			_apply_transport_passenger_damage(carrier_id, passenger_id, 2147483647)
			_sim.exit_entity_container(passenger_id)
