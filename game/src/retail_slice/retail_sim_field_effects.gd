...
func _attach_fire_weapon_when_dead_contract(row: Dictionary, contract: Dictionary) -> void:
	...
	var starts_value: Variant = _sim._module_contract_value(fields, "StartsActive", null)
	if typeof(starts_value) != TYPE_BOOL or not bool(starts_value):
		return
	var weapon_value: Variant = _sim._module_contract_value(fields, "DeathWeapon", "")
	var weapon_id := String(weapon_value).strip_edges()
	if weapon_id == "":
		return
	var delay_value: Variant = _sim._module_contract_value(fields, "DelayTime", 0.0)
	if typeof(delay_value) not in [TYPE_INT, TYPE_FLOAT]:
		return
	var offset := Vector2.ZERO
	var offset_z := 0.0
	var offset_value: Variant = _sim._module_contract_value(fields, "WeaponOffset", {})
	...
	rows.append({
		...
		"required_status": Array(_sim._module_contract_value(fields, "RequiredStatus", [])).duplicate(),
		"exempt_status": Array(_sim._module_contract_value(fields, "ExemptStatus", [])).duplicate(),
		"active_during_construction": bool(_sim._module_contract_value(fields, "ActiveDuringConstruction", false)),
		"delay_ticks": maxi(0, roundi(float(delay_value) / (_sim.TICK_SECONDS * 1000.0))),
...
func _configure_death_weapon_rules_from_rules() -> void:
	var _sim = sim
	_sim._death_weapon_rules.clear()
	var configured: Variant = _sim._rules.get("death_weapon_rules", {})
...
func _step_passive_area_effect_heals() -> void:
	...
	for structure_id in _sim.structure_ids():
		...
		if not bool(structure.get("structure_module_contracts_attached", false)):
			_sim._attach_structure_module_contracts(structure)
		if (
		...
		for rule_index in rules.size():
			...
			if upgrade_required != "" and not _structure_has_completed_upgrade(
				...
				continue
			var next_ping = int(rule.get("next_ping_tick", _sim.tick_index + 1))
			var ping_ticks := maxi(1, int(rule.get("ping_ticks", 1)))
			var due = _sim.tick_index >= next_ping
			if due:
				...
				while next_ping <= _sim.tick_index:
			...
			for entity_id in _sim.living_ids(team):
	...
	for entity_id_value in candidates.keys():
		...
		for candidate_value in stackable:
			_apply_passive_area_effect_heal(
				_sim.entities[entity_id] as Dictionary, candidate_value as Dictionary
			)
	...
	for structure_id in _sim.structure_ids():
		...
		for rule_index in rules.size():
			...
			if upgrade_required != "" and not _structure_has_completed_upgrade(structure, upgrade_required):
				continue
			var next_ping = int(rule.get("next_ping_tick", _sim.tick_index))
			if _sim.tick_index < next_ping:
				continue
			var ping_ticks := maxi(1, int(rule.get("ping_ticks", 1)))
			while next_ping <= _sim.tick_index:
				next_ping += ping_ticks
			...
			for entity_id in _sim.living_ids(team):
				var target: Dictionary = _sim.entities[entity_id]
				if not _sim._ability_filter_accepts(target, String(rule.get("allow_filter", ""))):
					continue
				if Vector2(target.get("position", Vector2.ZERO)).distance_to(origin) > radius:
					continue
				if bool(stacking.get("ignoreIfAnticategoryActive", false)) and category == "LEADERSHIP" and _sim._refresh_leadership_suppression(target) > _sim.tick_index:
					continue
				...
				if bool(stacking.get("replaceInCategoryIfLongest", false)) or bool(rule.get("non_stackable", false)):
					...
					if int(current.get("expires_tick", -1)) > _sim.tick_index + duration_ticks:
						continue
				_sim._set_timed_modifier(target, key, rule.get("effects", []) as Array, _sim.tick_index + duration_ticks)
				_sim._emit_event("module.passive_area_effect_modifier", structure_id, entity_id, {"modifier_id": modifier_id, "category": category})
		if rules.is_empty():
...
func _refresh_banner_carrier_state(row: Dictionary) -> void:
	...
	var _sim = sim
	var rule: Dictionary = _sim._unit_banner_carriers.get(String(row.get("unit_type", "")), {}) as Dictionary
	if rule.is_empty():
		rule = _sim._unit_banner_carriers.get(String(row.get("object_id", "")), {}) as Dictionary
	if rule.is_empty():
		return
	...
	if _sim.entity_container.has(int(row.get("id", 0))):
		var container_id = int(_sim.entity_container[int(row.get("id", 0))])
		if _sim.structures.has(container_id) and not _sim.structure_allows_banner_spawn(container_id):
			return
	var banner_id := int(row.get("banner_entity_id", 0))
	if banner_id != 0 and _sim.entities.has(banner_id) and int((_sim.entities[banner_id] as Dictionary).get("health", 0)) > 0:
...
func _spawn_banner_carrier_entity(parent: Dictionary, rule: Dictionary) -> void:
	...
	if not _sim._next_dynamic_id.has(team):
		_sim._next_dynamic_id[team] = 900000 + team * 1000
	...
	var at = Vector2(parent.get("position", Vector2.ZERO)) + _sim._retail_source_to_sim_offset(offset_source)
	...
	if not _sim.entities.has(entity_id):
		return
	var banner: Dictionary = _sim.entities[entity_id]
	if not banner.has("module_contracts"):
		_sim._attach_module_contracts(banner)
	...
	_sim._emit_event("battalion.banner_spawned", int(parent.get("id", 0)), entity_id, {
		"team": team,
...
func _sync_banner_entity_transform(parent: Dictionary, banner_id: int, rule: Dictionary) -> void:
	var _sim = sim
	if not _sim.entities.has(banner_id):
		return
	var banner: Dictionary = _sim.entities[banner_id]
	var offset_source: Vector2 = rule.get("offset_source", Vector2.ZERO)
	banner["position"] = Vector2(parent.get("position", Vector2.ZERO)) + _sim._retail_source_to_sim_offset(offset_source)
	...
	for id in _sim.entity_ids():
		...
		if bool(row.get("is_banner_carrier", false)):
			...
			if int(row.get("health", 0)) <= 0:
				...
				continue
			if parent_id == 0 or not _sim.entities.has(parent_id) or int((_sim.entities[parent_id] as Dictionary).get("health", 0)) <= 0:
				_sim.entities.erase(id)
				continue
			if parent_id != 0 and _sim.entities.has(parent_id):
				var parent: Dictionary = _sim.entities[parent_id]
				var rule: Dictionary = _sim._unit_banner_carriers.get(String(parent.get("unit_type", "")), {}) as Dictionary
				if rule.is_empty():
					rule = _sim._unit_banner_carriers.get(String(parent.get("object_id", "")), {}) as Dictionary
		...
		if remaining < 0:
			...
			continue
		if remaining > 0:
			if int(row.get("banner_respawn_armed_tick", -1)) == _sim.tick_index:
				continue
...
func _on_banner_carrier_defeated(banner: Dictionary) -> void:
	...
	if parent_id == 0 or not _sim.entities.has(parent_id):
		return
	var parent: Dictionary = _sim.entities[parent_id]
	...
	if bool(parent.get("banner_carrier_destroy_horde_on_death", false)):
		...
		_sim._emit_event("battalion.defeated", int(banner.get("id", 0)), parent_id, {
	...
	if typed_update.is_empty():
		_sim._attach_module_contracts(banner);typed_update=banner.get("banner_carrier_update",{}) as Dictionary
	...
	if _sim._banner_respawn_ticks_by_object.has(banner_object_id):
		respawn_ticks = int(_sim._banner_respawn_ticks_by_object[banner_object_id])
	...
	if respawn_ticks < 0 and not rule.is_empty():
		...
		if source_name != "" and _sim._banner_respawn_ticks_by_object.has(source_name):
			respawn_ticks = int(_sim._banner_respawn_ticks_by_object[source_name])
		var rule_oid := String(rule.get("object_id", ""))
		if respawn_ticks < 0 and rule_oid != "" and _sim._banner_respawn_ticks_by_object.has(rule_oid):
			respawn_ticks = int(_sim._banner_respawn_ticks_by_object[rule_oid])
	if respawn_ticks >= 0:
		parent["banner_respawn_ticks_remaining"] = respawn_ticks
		parent["banner_respawn_armed_tick"] = _sim.tick_index
		_sim._emit_event("battalion.banner_respawn_armed", parent_id, int(banner.get("id", 0)), {
...
func _step_banner_replenishment(banner:Dictionary)->void:
	...
	if _sim.tick_index<int(update.get("next_replenish_tick",0)):return
	update["next_replenish_tick"]=_sim.tick_index+maxi(1,int(update.get("idle_spawn_ticks",1)));banner["banner_carrier_update"]=update
	var parent_id:=int(banner.get("banner_parent_entity_id",0));var required:=String(update.get("upgrade_required",""))
	if required!="" and (parent_id==0 or not _sim.entities.has(parent_id) or not _structure_has_completed_upgrade(_sim.entities[parent_id] as Dictionary,required)):return
	...
	if bool(update.get("replenish_all",false)):
		var radius=float(update.get("scan_range_source",0.0))*float(_sim._rules.get("source_unit_scale",0.1));var origin=Vector2(banner.get("position",Vector2.ZERO))
		for id in _sim.entity_ids():
			if id==int(banner.get("id",0)) or candidates.has(id):continue
			if int((_sim.entities[id] as Dictionary).get("team",-1))!=int(banner.get("team",-1)):continue
			if radius<=0.0 or origin.distance_to(Vector2((_sim.entities[id] as Dictionary).get("position",Vector2.ZERO)))<=radius:candidates.append(id)
	for id in candidates:
		if not _sim.entities.has(id):continue
		var target=_sim.entities[id] as Dictionary;var members=target.get("member_health",[]) as Array;var replenished=-1
		...
		_sim._emit_event("battalion.banner_replenished",int(banner.get("id",0)),id,{"member_index":replenished})
