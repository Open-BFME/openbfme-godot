...
## bold title, an optional "Cost: N" treasure row, an optional "Shortcut: X"
## line, and one or two description lines. Colors are sampled from the retail
## palette already used by retail_hud.gd (dark #0a0a08 background, gold
## #d9c996 / #f1d06e accents). Text never invents lore: callers pass strings
## sourced from the pack's strings.json (or the spec fallbacks).
...
func _build() -> void:
	...
	_header_row.add_child(_shortcut_label)
...
func set_retail_font(font: FontFile) -> void:
	...
	if font == null:
		return
	for label in [_title_label, _shortcut_label, _cost_label, _description_label]:
...
## description: 1-2 lines of pack description text.
func show_content(title: String, cost: int, shortcut: String, description: String) -> void:
	if _panel == null:
		_build()
