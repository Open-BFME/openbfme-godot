...
func cast_power(team: int, power_id: String, point: Vector2) -> Dictionary:
	...
	if not sim.has_power(team, power_id):
	...
	var cooldown = sim.power_cooldown_state(team, power_id)
	...
	sim._emit_event("power.cast", 0, 0, {
		...
		"fx_radius": snappedf(
			float(effect.get("radius_source", effect.get("range_source", 0.0))) * sim._spellbook_world_scale(),
			0.001
		),
...
func _cast_spellbook_heal(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	var radius = float(effect.get("radius_source", 0.0)) * sim._spellbook_world_scale()
...
func _cast_spellbook_attribute_modifier(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	var range_sim = float(effect.get("range_source", 0.0)) * sim._spellbook_world_scale()
...
func _cast_spellbook_structure_summon(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	sim.structures[structure_id] = {
		...
		"attack": {
			...
			"range": float(weapon.get("range_source", 0.0)) * sim._spellbook_world_scale(),
...
func _cast_spellbook_grove(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	sim._active_groves.append({
		"team": team,
		"point": point,
		"range_sim": float(effect.get("range_source", 0.0)) * sim._spellbook_world_scale(),
	...
	for ping in sim._field_pings:
		...
		for aura_value in Array(ping.get("auras", [])):
			...
			if sim.tick_index % maxi(1, int(aura.get("refresh_ticks", 1))) != 0:
				continue
			var radius = float(aura.get("range_source", 0.0)) * sim._spellbook_world_scale()
		...
		for policy_value in ping.get("invisibility_updates", []) as Array:
			...
			var radius = float(policy.get("broadcast_range_source", 0.0)) * sim._spellbook_world_scale(); var filter_tokens = policy.get("broadcast_filter", []) as Array
...
func _cast_spellbook_creep_allegiance(team: int, effect: Dictionary, point: Vector2) -> Dictionary:
	...
	## are not.
	var radius = float(effect.get("range_source", 0.0)) * sim._spellbook_world_scale()
