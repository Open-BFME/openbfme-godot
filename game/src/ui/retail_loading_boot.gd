...
func _ready() -> void:
	...
	if packed != null:
		...
		add_child(_screen)
	if ResourceLoader.load_threaded_request(SLICE_SCENE_PATH) == OK:
		_requested = true
	else:
		# Fallback: synchronous load keeps the transition functional.
		_apply_loaded_scene(load(SLICE_SCENE_PATH))
...
func _process(_delta: float) -> void:
	...
	var status := ResourceLoader.load_threaded_get_status(SLICE_SCENE_PATH, progress)
	if status == ResourceLoader.THREAD_LOAD_IN_PROGRESS:
		_screen.call("set_load_progress", float(progress[0]) * SCENE_LOAD_SHARE, "Loading match scene")
	elif status == ResourceLoader.THREAD_LOAD_LOADED:
		_requested = false
		_apply_loaded_scene(ResourceLoader.load_threaded_get(SLICE_SCENE_PATH))
