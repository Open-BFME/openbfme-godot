...
const GAP_RECRUIT_UNITS := (
	...
	+ "The INT is a second, independent problem: the source reference's "
	+ "description for these two entries is garbled by placeholder substitution "
	+ "('will recruit <TEAM> units of type <INT> from nearby recruitable allied "
	+ "teams.<OBJECT_TYPE_LIST>'), so whether it is a unit COUNT or a recruiting "
	+ "RADIUS is not established - the sibling RECRUIT_TEAM(TEAM, REAL) spells "
	+ "its radius REAL and in feet, which argues for count, but that is an "
	+ "inference and this file does not guess a semantic. NEEDED: a signature "
	+ "carrying the type list, plus a ruling on the INT)"
)
