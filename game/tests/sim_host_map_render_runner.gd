...
func _run() -> void:
	...
	while Time.get_ticks_msec() - started < 30_000:
		...
		if elapsed_seconds >= next_fps_second:
			print("SIM_HOST_MAP_RENDER_FPS seconds=%d fps=%.2f" % [next_fps_second, Engine.get_frames_per_second()])
			next_fps_second += 5
		if capture_index < CAPTURE_SECONDS.size() and elapsed_seconds >= int(CAPTURE_SECONDS[capture_index]):
			...
			var path := _repo_path("workspace/logs/lane-map-scene/fords-%02ds.png" % seconds)
