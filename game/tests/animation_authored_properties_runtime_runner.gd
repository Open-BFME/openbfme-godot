...
func _runtime_receipt_test(capability: Dictionary) -> void:
	...
	_check(idle_receipt.get("animationPriorityApplied") == true, "priority is applied as equal-specificity tie-break")
	_check(String(idle_receipt.get("animationPriorityRuntimeSupport", "")) == "higher-wins-equal-specificity", "priority law is higher-wins")
