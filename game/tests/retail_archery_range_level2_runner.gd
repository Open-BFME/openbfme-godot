...
func _initialize() -> void:
	_runner_watchdog.start(self, "RETAIL_ARCHERY_RANGE_LEVEL2_RUNNER")
	call_deferred("_run")
...
func _load_ranger_contract() -> Dictionary:
	var path := OS.get_environment(PROFILE_ENV).strip_edges()
	if path == "":
		_fail(
			"source_profile_configured",
			"Set %s to the generated ranger profile JSON." % PROFILE_ENV
		)
		return {}
