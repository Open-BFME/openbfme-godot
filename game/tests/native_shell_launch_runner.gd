...
func _run() -> void:
	...
	_check("map_from_native_index", String((document.get("map", {}) as Dictionary).get("path", "")) == String(expected_map.get("path", "")) and not String(expected_map.get("path", "")).is_empty())
	...
	if players.size() == 2:
		...
		_check("colors_and_starts", int((players[0] as Dictionary).get("color", -1)) == 2 and int((players[1] as Dictionary).get("color", -1)) == 4 and int((players[0] as Dictionary).get("start_position", -1)) == 0 and int((players[1] as Dictionary).get("start_position", -1)) == 1)
	...
	boot.call("_apply_loaded_scene", load("res://scenes/sim_host_match.tscn"))
	...
	_check("launch_survives_transition", game_state != null and (game_state.get_meta("native_match_launch", {}) as Dictionary) == document)
