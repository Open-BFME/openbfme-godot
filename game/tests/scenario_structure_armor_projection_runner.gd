...
func _run() -> void:
	...
	_check((sim._structure_armor.get("captureflag", {}) as Dictionary).get("scalars", {}) == {"default": 1.0}, "explicit null armor projects unmodified damage")
	...
	_check(capture_id > 0 and String((sim.structures[capture_id] as Dictionary).get("structure_kind", "")) == "captureflag", "spawn reuses normalized neutral structure kind")
