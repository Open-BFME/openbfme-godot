...
func _run() -> void:
	...
	# Keep this generic runtime contract probe independent of whichever immutable
	# pack is selected. It proves only that an admitted synthetic replacement
	# template executes; canonical retail ownership is a separate evidence claim.
	live._ensure_parity()
	...
	live_rules["replace_self_structure_templates"] = {
		"FixtureWallSegment": {"structure_kind": "wall_segment", "maximum_health": 1500},
	}
	live.setup({}, live_rules)
	var live_spec := live._replace_self_structure_spec(0, "FixtureWallSegment")
	...
	_check("repeated_additions_order_preserved", ((wall.get("replace_self_upgrades", []) as Array)[0] as Dictionary).get("and_then_add", []) == ["FixtureWallHub", "FixtureWallSegment"])
	...
	_check("replacement_object_identity_exact", String(replaced.get("source_object_id", "")) == "FixtureWallSegment")
	...
	_check("prior_upgrades_preserved", (replaced.get("completed_upgrades", []) as Array) == ["Upgrade_FixtureWallHub", "Upgrade_Common"])
	...
	_check("additional_order_exact", String((sim.structures[3000] as Dictionary).get("source_object_id", "")) == "FixtureWallHub" and String((sim.structures[3001] as Dictionary).get("source_object_id", "")) == "FixtureWallSegment")
	...
	_check("replacement_state_round_trips", String((restored.structures[50] as Dictionary).get("source_object_id", "")) == "FixtureWallSegment" and restored.structures.has(3001))

	var conflict := _sim()
	var conflict_wall := conflict.structures[50] as Dictionary
	conflict_wall["completed_upgrades"] = ["Upgrade_FixtureWallHub", "Upgrade_FixtureWallGate"]
	conflict._attach_replace_self_contract(conflict_wall, _contract())
	conflict._step_replace_self_upgrades()
	_check("conflict_blocks_replacement", String((conflict.structures[50] as Dictionary).get("source_object_id", "")) == "FixtureWallUpgradeNode" and conflict.structures.size() == 1)
	var direct := _sim()
	...
	_check("command_api_grants_trigger_and_replaces", bool(direct.apply_replace_self_upgrade(50, "Upgrade_FixtureWallHub").get("ok", false)) and String((direct.structures[50] as Dictionary).get("source_object_id", "")) == "FixtureWallSegment")
	...
	_check("unresolved_template_fails_closed", String(unresolved_wall.get("source_object_id", "")) == "FixtureWallUpgradeNode" and not bool(unresolved_policy.get("consumed", false)))
...
func _contract() -> Dictionary:
	return {"module": "ReplaceSelfUpgrade", "extraction": "typed", "tag": "ModuleTag_Replace", "fields": {"ReplaceWith": {"value": "FixtureWallSegment"}, "AndThenAddA": [{"value": "FixtureWallHub", "line": 101}, {"value": "FixtureWallSegment", "line": 102}], "TriggeredBy": {"value": "Upgrade_FixtureWallHub"}, "ConflictsWith": {"value": ["Upgrade_FixtureWallGate", "Upgrade_FixtureWallPostern", "Upgrade_FixtureWallTower", "Upgrade_FixtureWallTrebuchet"]}}}
...
func _sim() -> RetailSliceSim:
	...
	sim.setup({}, {"unit_rules": unit_rules, "faction_manifest": {"structure_armor": _fixture_structure_armor()}, "replace_self_structure_templates": {"FixtureWallSegment": {"structure_kind": "wall_segment", "maximum_health": 200}, "FixtureWallHub": {"structure_kind": "wall_hub", "maximum_health": 300}}})
	sim.ai_enabled = false
	sim.base_loop_enabled = false
	sim.entities.clear()
	sim.structures.clear()
	sim.structures[50] = {"id": 50, "team": 2, "kind": "structure", "structure_kind": "wall_upgrade_node", "source_object_id": "FixtureWallUpgradeNode", "name": "Wall upgrade node", "position": Vector2(4, 4), "rally": Vector2(8, 4), "facing": Vector2.RIGHT, "health": 50, "maximum_health": 100, "construction_progress": 1.0, "level": 1, "completed_upgrades": ["Upgrade_FixtureWallHub", "Upgrade_Common"], "upgrade_queue": [], "production": [], "queue": [], "damage_remainders": {}, "income_per_payout": 0}
