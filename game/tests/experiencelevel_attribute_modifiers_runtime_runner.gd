extends SceneTree
## definition-kind:ExperienceLevel; field:experiencelevel.AttributeModifiers
...
func _run() -> void:
	_report(Support.new().run(self, "field:experiencelevel.AttributeModifiers"))


func _report(result: Dictionary) -> void:
	_reported = true
	var ok := bool(result.get("ok", false))
	print("EXPERIENCE_MODIFIERS_RESULT passed=%d failed=%d detail=%s" % [
